...
class DeepseekMLP(nn.Module):
    def __init__(self, config, hidden_size=None, intermediate_size=None, parallel=False, all_reduce=True):
        super().__init__()
        self.config = config
    ...
    def forward(self, x):
        ...
        if self.parallel and self.world_size >= 2 and self.all_reduce:
            torch.distributed.all_reduce(down_proj, op=torch.distributed.ReduceOp.SUM,
...
class DeepseekMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.expert_parallel and self.world_size > 1:
            experts = [DeepseekMLP(config, intermediate_size=config.moe_intermediate_size,
                                   parallel=False, all_reduce=not self.all_reduce)
                       for i in range(config.n_routed_experts)]
            self.expert_indicies = np.array_split(range(config.n_routed_experts), self.world_size)[self.rank].tolist()
            self.experts = nn.ModuleList([e if i in self.expert_indicies else None for i, e in enumerate(experts)])
        else:
            experts = [DeepseekMLP(config, intermediate_size=config.moe_intermediate_size,
                                   parallel=True, all_reduce=not self.all_reduce)
                       for i in range(config.n_routed_experts)]
            self.experts = nn.ModuleList(experts)
        self.gate = MoEGate(config)
        if config.n_shared_experts is not None:
            intermediate_size = config.moe_intermediate_size * config.n_shared_experts
            self.shared_experts = DeepseekMLP(config=config, intermediate_size=intermediate_size, parallel=True)
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            if self.expert_parallel:
                y = self.moe_ep_infer(hidden_states, flat_topk_idx, topk_weight.view(-1, 1)).view(*orig_shape)
            else:
                y = self.moe_infer(hidden_states, flat_topk_idx, topk_weight.view(-1, 1)).view(*orig_shape)

        if self.all_reduce:
            torch.distributed.all_reduce(y, op=torch.distributed.ReduceOp.SUM, group=get_tensor_model_parallel_group())
    ...
    @torch.no_grad()
    def moe_ep_infer(self, x, flat_expert_indices, flat_expert_weights):
        expert_cache = torch.zeros_like(x)
        idxs = flat_expert_indices.argsort()
        tokens_per_expert = flat_expert_indices.bincount().cpu().numpy().cumsum(0)
        token_idxs = idxs // self.num_experts_per_tok
        for i, end_idx in enumerate(tokens_per_expert):
            start_idx = 0 if i == 0 else tokens_per_expert[i - 1]
            if start_idx == end_idx or i not in self.expert_indicies:
                continue
            expert = self.experts[i]
            exp_token_idx = token_idxs[start_idx:end_idx]
            expert_tokens = x[exp_token_idx]
...
class DeepseekAttention(nn.Module):
    ...
    def __init__(self, config: DeepseekConfig, layer_idx: Optional[int] = None):
        ...
        if (self.head_dim * self.num_heads * self.world_size) != self.hidden_size:
            raise ValueError(
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
        ...
        kv_seqs_len = key_states.shape[-2]
        if past_key_value is not None:
            ...
            kv_seqs_len += past_key_value.get_usable_length(kv_seqs_len, self.layer_idx)

        cos, sin = self.rotary_emb(value_states, seq_len=kv_seqs_len)
        ...
        if attn_weights.size() != (bsz, self.num_heads, q_len, kv_seqs_len):
            raise ValueError(
                f"Attention weights should be of size {(bsz, self.num_heads, q_len, kv_seqs_len)}, but is"
                f" {attn_weights.size()}"
            )

        if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, q_len, kv_seqs_len):
                raise ValueError(
                    f"Attention mask should be of size {(bsz, 1, q_len, kv_seqs_len)}, but is {attention_mask.size()}"
                )
...
class DeepseekDecoderLayer(nn.Module):
    def __init__(self, config: DeepseekConfig, layer_idx: int):
        ...
        self.mlp = DeepseekMoE(config) if (config.n_routed_experts is not None and \
            ...
            else DeepseekMLP(config, parallel=True)
...
class DeepseekModel(DeepseekPreTrainedModel, PipeBaseModule):
    """
    ...
    def __init__(self, config: DeepseekConfig):
        ...
        layers = self.process_pipe([DeepseekDecoderLayer(config, layer_idx)
                                    for layer_idx in range(config.num_hidden_layers)])

        for _, layer in enumerate(layers):
            if layer is None:
                continue
            layer.self_attn.layer_idx -= self.local_stage_layers_idx[0]
    ...
    def first_stage_process(self, input_ids):
        inputs_embeds = self.embed_tokens(input_ids)
        return inputs_embeds

    def last_stage_process(self, h):
        return self.norm(h)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.recv(input_ids)
        ...
        for idx in self.local_stage_layers_idx:
        ...
        hidden_states = self.send(hidden_states)
...
def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
    ...
    rank_mp = get_tensor_model_parallel_rank()
        ...
        if key in state_dict:
            ...
            if input_param.shape != param.shape:
                ...
                if input_param.dim() == param.dim():
                    for dim in range(input_param.dim()):
                        if input_param.size(dim) != param.size(dim) and input_param.size(dim) % param.size(dim) == 0:
                    ...
                    if len(diff_dims) > 1:
                        ...
                        continue
                    elif len(diff_dims) == 0:
                        pass
                    else:
                        size = param.size(diff_dims[0])
                        shape = input_param.shape
                        input_param = input_param.split(size, dim=diff_dims[0])[rank_mp]
                else:
                    error_msgs.append('size mismatch for {}: copying a param with shape {} from checkpoint, '
                                      'the shape in current model is {}.'
                                      .format(key, input_param.shape, param.shape))
...
def replace_load_func(self):
    self._load_from_state_dict = functools.partial(_load_from_state_dict, self)
...
class DeepseekForCausalLM(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        if self.world_size > 1:
            ...
            logits = [torch.zeros_like(out) for _ in range(self.world_size)]
            torch.distributed.all_gather(logits, out, group=get_tensor_model_parallel_group())
