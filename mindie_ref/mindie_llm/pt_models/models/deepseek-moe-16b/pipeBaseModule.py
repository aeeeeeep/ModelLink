...
def initialize_model_parallel(tp: int = 1, pp: int = 1) -> None:
    ...
    if world_size != (tp * pp):
        raise RuntimeError(
            f"world_size ({world_size}) is not divisible by tp "
            f"({tp}) x pp ({pp})"
        )

    for index in range(0, world_size, tp):
        ranks = range(index, index + tp)
        group = torch.distributed.new_group(ranks)
        if rank in ranks:
            _TENSOR_MODEL_PARALLEL_GROUP_CURRENT = group

    for index in range(tp):
        ranks = range(index, world_size, tp)
        group = torch.distributed.new_group(ranks)
        if rank in ranks:
...
def get_pipeline_model_parallel_prev_rank():
    ...
    return _PIPELINE_GLOBAL_RANKS_CURRENT[(rank - 1) % world_size]
