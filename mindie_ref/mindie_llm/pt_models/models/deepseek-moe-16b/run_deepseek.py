    ...
    for batch in batch_list:
        ...
        for seqlen_in in seq_in_list:
            for seqlen_out in seq_out_list:
                ...
                torch.npu.synchronize()
                if torch.distributed.get_rank() == 0:
                    print("-----------------Batch, seq_in, seq_out------------------------", batch, seqlen_in,
                          seqlen_out)
                    save_path = "mixtral8x7b_performance/"
                    old_name = save_path + "time"
                    current_date = datetime.datetime.now()
                    formatted_date = current_date.strftime("%Y-%m-%d %H:%M:%S")
                    new_name = save_path + formatted_date + "_SIN_" + str(seqlen_in) + "_SOUT_" + str(
                        seqlen_out) + "_BS_" + str(batch)
