...
class BaseLauncher:
    ...
    def __init__(self, device_ids: str = None, model_path="", options=None):
        ...
        self.logger.info(f"load model from {os.path.basename(inspect.getmodule(self.model).__file__)} successfully!")
        self.logger.info(f"load model from {os.path.realpath(inspect.getmodule(self.model).__file__)} successfully!")
    ...
    def infer(self, query, model_params=None):
        ...
        self.logger.info(f"cost {time_cost}s")
    ...
    @staticmethod
    def remove_part_of_generation_config(generation_config) -> GenerationConfig:
        """
        移除部分当前不支持后处理相关参数
