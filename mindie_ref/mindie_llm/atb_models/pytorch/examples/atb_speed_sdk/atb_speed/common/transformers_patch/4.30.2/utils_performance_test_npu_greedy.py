...
RETURN_PERF_DETAIL = int(os.getenv("RETURN_PERF_DETAIL", 0))
...
class GenerationMixin:
    ...
    def greedy_search(
        ...
        while True:
            ...
            # perprocess
            elapse = (forward_start_time - pre_start_time) * 1000  # ms
            if forward_count == 1:
            ...
            # forward
            elapse = (forward_end_time - forward_start_time) * 1000  # ms
            if forward_count == 1:
            ...
            # print(f"next_tokens[:, None] is {next_tokens[:, None]}")
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            ...
            elapse = (postprocess_end_time - forward_end_time) * 1000  # ms
            if forward_count == 1:
        ...
        pre_next_token_time = pre_total_token_time / (forward_count - 1) if forward_count - 1 else 0
        print(f"===========pre process time==============\n")
        print(f"[first token delay]: {pre_first_token_time}ms\n")
        print(f"[avg token delay]: {pre_next_token_time}ms\n")
        print(f"[all token delay]: {pre_total_token_time + pre_first_token_time}ms\n")

        forward_next_token_time = forward_total_token_time / (forward_count - 1) if forward_count - 1 else 0
        print(f"===========model process time==============\n")
        print(f"[first token delay]: {forward_first_token_time}ms\n")
        print(f"[avg token delay]: {forward_next_token_time}ms\n")
        print(f"[all token delay]: {forward_total_token_time + forward_first_token_time}ms\n")

        post_next_token_time_post = post_total_token_time / (forward_count - 1) if forward_count - 1 else 0
        print(f"===========post process time==============\n")
        print(f"[first token delay]: {post_first_token_time}ms\n")
        print(f"[avg token delay]: {post_next_token_time_post}ms\n")
        print(f"[all token delay]: {post_total_token_time + post_first_token_time}ms\n")
        ...
        if return_dict_in_generate:
            ...
            if not RETURN_PERF_DETAIL:
                return input_ids
            else:
                return input_ids, forward_first_token_time, forward_next_token_time, pre_next_token_time, post_next_token_time_post
