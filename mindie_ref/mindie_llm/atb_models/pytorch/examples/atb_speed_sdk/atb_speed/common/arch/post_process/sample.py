...
from transformers.generation import BeamScorer, BeamSearchScorer, ConstrainedBeamSearchScorer
...
class SampleEncoderDecoderOutput(ModelOutput):
    ...
    Args:
        ...
        scores (`tuple(torch.FloatTensor)` *optional*, returned when `output_scores=True` is passed
            or when `config.output_scores=True`):
...
class SampleForModelsBase:
    def __init__(self,
        ...
        self._update_model_kwargs_for_generation = _update_model_kwargs_for_generation
    ...
    def sample(
        ...
        In most cases, you do not need to call [`~generation.GenerationMixin.sample`] directly.
        ...
        Parameters:
            ...
            logits_processor (`LogitsProcessorList`, *optional*):
                ...
                List of instances of class derived from [`LogitsProcessor`] used to modify the
                prediction scores of the language modeling head applied at each generation step.
            ...
            logits_warper (`LogitsProcessorList`, *optional*):
                An instance of [`LogitsProcessorList`].
                List of instances of class derived from [`LogitsWarper`] used to warp the
                prediction score distribution of the language modeling head applied
            ...
            eos_token_id (`Union[int, List[int]]`, *optional*):
                The id of the *end-of-sequence* token. Optionally, use a list to set multiple
        ...
        Return:
            [`~generation.SampleDecoderOnlyOutput`], [`~generation.SampleEncoderDecoderOutput`]
            or `torch.LongTensor`:
        ...
        seed = torch.npu.initial_seed()
        mod_max_int32 = 4294967295
        seed = seed % mod_max_int32 # 由于seed是int64，randSeed只支持传uint32
        temperature = self.sample_config.get("temperature", 1.0)
        topk = self.sample_config.get("top_k", 50)
        topp = self.sample_config.get("top_p", 1.0)
        sample_operation = torch.classes.OperationTorch.OperationTorch("common_SampleLayerCreate")
        sample_param = json.dumps({"temperature": temperature, "topK": topk, "randSeed": seed})
        sample_operation.set_param(sample_param)            
        ...
        while True:
            ...
            # new sample
            temperature_in = torch.HalfTensor([temperature]).npu()
            topp_in = torch.HalfTensor([topp]).npu()
            indices_out, probs_out = sample_operation.execute([next_token_scores, temperature_in, topp_in])
            next_tokens = indices_out.squeeze(1)
