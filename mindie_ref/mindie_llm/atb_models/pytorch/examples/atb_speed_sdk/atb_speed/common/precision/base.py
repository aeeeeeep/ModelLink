...
from atb_speed.common.utils import torch_parallel_info, print_rank_0
...
class Record:
    ...
    def load_cache(self):
        ...
        with open(self.cache_name, "r", encoding="utf-8") as f:
    ...
    def update_cache(self, task_name, question_id, truth_answer, predict_answer):
        if self.debug or torch_parallel_info.is_rank_0:
            with open(self.cache_name, "a", encoding="utf-8") as f:
...
class PrecisionTestBase:
    ...
    def compute_metric(self, subject_mapping):
        ...
        with open(os.path.join(self.result_output_dir, f"result{self.recorder.flag}_subject_acc.json"), "w") as fp:
            json.dump(subject_acc, fp)
        with open(os.path.join(self.result_output_dir, f"result{self.recorder.flag}_classes_acc.json"), "w") as fp:
            json.dump(classes_acc, fp)
        print_rank_0(f"[+] Avg acc: {classes_acc['Avg']}")

    def get_subject_mapping(self):
        with open(self.subject_mapping_path, encoding="utf-8") as f:
    ...
    def batch_infer(self, qr_pair, begin_idx):
        ...
        for prompt, truth_answer, task_name, answer in zip(prompts, truth_answers, task_names, answers):
            self.recorder.log("\n========== prompt start ==========\n", prompt,
                              "\n==========  prompt end  ==========\n")
