...
def bias_correction(fp_bias_dict, quant_weight_dict, input_offset_dict, deq_scale_dict):
    ...
    for key in fp_bias_dict.keys():
        new_bias_dict[key] = fp_bias_dict[key].npu()/deq_scale_dict[key].npu() - \
            quant_weight_dict[key].to(torch.float32).npu().sum(
                dim=1) * float(input_offset_dict[key])
...
def cut_quant_weights(model_cfg, weights, tp_size):
    ...
    for key, tensor in weights.items():
        cut_tensor_list = cut_quant_tensors(
            model_cfg, key, tensor, tp_size, mode="weights")
        for i in range(tp_size):
            state_dict_list[i][key] = cut_tensor_list[i]
    return state_dict_list
...
def cut_quant_bias(model_cfg, bias, tp_size):
    ...
    for key, tensor in bias.items():
        cut_tensor_list = cut_quant_tensors(
            model_cfg, key, tensor, tp_size, mode="bias")
        for i in range(tp_size):
            state_dict_list[i][key] = cut_tensor_list[i]
    return state_dict_list
...
def cut_quant_scales(model_cfg, bias, tp_size):
    ...
    for key, tensor in bias.items():
        cut_tensor_list = cut_quant_tensors(
            model_cfg, key, tensor, tp_size, mode="scales")
        for i in range(tp_size):
            state_dict_list[i][key] = cut_tensor_list[i]
    return state_dict_list
...
if __name__ == "__main__":
    ...
    shutil.copy(args.model_file_golden, os.path.join(
        args.model_path, "modeling_chatglm.py"))
    model = AutoModel.from_pretrained(
        args.model_path, trust_remote_code=True).half()
            ...
            for i in range(args.tp_size):
                ...
                for source_file in ["configuration_chatglm.py", "quantization.py"]:
                    shutil.copy(os.path.join(
                        args.model_path, source_file), target_dir)
            print(
                f"[info]: The parallel float weights has been saved to '{save_path}'.")

    # cut or correct quant weights
    if ENABLE_QUANT or ENABLE_SPARSE:
        # load quant weights
        QUANT_WEIGHT_PATH = os.environ.get("QUANT_WEIGHT_PATH")
        input_offset_dict = np.load(os.path.join(
            QUANT_WEIGHT_PATH, "input_offset.npy"), allow_pickle=True).item()
        quant_weight_dict = np.load(os.path.join(
            QUANT_WEIGHT_PATH, "quant_weight.npy"), allow_pickle=True).item()
        deq_scale_dict = np.load(os.path.join(
            QUANT_WEIGHT_PATH, "deq_scale.npy"), allow_pickle=True).item()
        fp_bias_dict = np.load(os.path.join(
            QUANT_WEIGHT_PATH, "fp_bias.npy"), allow_pickle=True).item()

        # correct bias and deq_scale
        bias_dict = bias_correction(
        ...
        state_quant_weight_dict_list = cut_fp_1st_layer_weights(
            model, args.tp_size)

        # handle quant weights of other layers
        for i in range(args.tp_size):
            np.save(os.path.join(QUANT_WEIGHT_PATH,
                                 f"weight{i}.npy"), state_quant_weight_dict_list[i])
            np.save(os.path.join(QUANT_WEIGHT_PATH,
                                 f"bias{i}.npy"), state_quant_weight_dict_list[i])
        print("[info]: save float weights of 1st layer")

        state_quant_weight_dict_list = cut_quant_weights(
            model_config, quant_weight_dict, args.tp_size)
        state_bias_dict_list = cut_quant_bias(
            model_config, bias_dict, args.tp_size)
        state_deq_scale_dict_list = cut_quant_scales(
            model_config, deq_scale_dict, args.tp_size)
        for i in range(args.tp_size):
            np.save(os.path.join(QUANT_WEIGHT_PATH,
                                 f"quant_weight{i}.npy"), state_quant_weight_dict_list[i])
            np.save(os.path.join(QUANT_WEIGHT_PATH,
                                 f"new_bias{i}.npy"), state_bias_dict_list[i])
            np.save(os.path.join(QUANT_WEIGHT_PATH,
                                 f"new_deq_scale{i}.npy"), state_deq_scale_dict_list[i])
        print("[info]: save quant weights of the other layers")
        print(
            f"[info]: The processed quant weights has been saved to '{QUANT_WEIGHT_PATH}'.")
