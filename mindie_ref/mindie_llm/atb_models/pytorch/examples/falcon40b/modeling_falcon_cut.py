...
class FalconRotaryEmbedding(nn.Module):
    ...
    def forward(self, query, key, past_key_values_length, position_ids):
        ...
        #         print(f"[!] nan: {torch.isnan(query).any()}")
        #         print(f"[!] inf: {torch.isinf(query).any()}")
        #         print(query.cpu())
        # except:
        #     print(query.cpu())
        # assert not (torch.isnan(query).any() or torch.isinf(query).any()), f"torch.nan: {torch.isnan(query).any()}, torch.nan: {torch.isinf(query).any()}"
        # assert not (torch.isnan(key).any() or torch.isinf(key).any()), "torch.nan in key or torch.inf in key"
...
class FalconAttention(nn.Module):
    def __init__(self, config: FalconConfig):
        ...
        #         f" {self.num_heads})."
        #     )
        print("config.rotary: ", config.rotary)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("[+] fused_qkv", fused_qkv.min(), fused_qkv.max())
        # global CNT
        # CNT += 1
        # print(f"CNT: {CNT}")
        if local_rank == 0:
            torch.save(hidden_states, f"results/hidden_states_{self.world_size}_{local_rank}.pt")
            torch.save(self.query_key_value.state_dict(), f"results/qkv_weight_{self.world_size}.bin")
            torch.save(fused_qkv, f"results/fused_qkv_{self.world_size}_{local_rank}.pt")
        ...
        if alibi is None:
            ...
            if local_rank == 0:
                # print("[+] output_tensor:", output_tensor.min(), output_tensor.max())
                torch.save(output_tensor, f"results/attn_output_tensor_after_reduce_{self.world_size}_{local_rank}.pt")
...
class FalconMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print("[+] mlp x1:", x.min(), x.max())
        x = self.act(self.dense_h_to_4h(x))
        # print("[+] mlp x2:", x.min(), x.max())
        x = self.dense_4h_to_h(x)
        # print("[+] mlp x3:", x.min(), x.max())
        if self.world_size >= 2:
            torch.distributed.all_reduce(x, op=torch.distributed.ReduceOp.SUM)
        local_rank = torch.distributed.get_rank()
        if local_rank == 0:
            torch.save(x, f"results/mlp_output_{self.world_size}_{local_rank}.pt")
        return x
        ...
        **kwargs,
        ...
        if not self.config.new_decoder_architecture:
            if self.config.parallel_attn:
                mlp_layernorm_out = attention_layernorm_out # true
            else:
...
class FalconForCausalLM(FalconPreTrainedModel):
    ...
    def forward(
        ...
        if local_rank == 0:
            ...
            torch.save(lm_logits, f"results/lm_logits_{WORLD_SIZE}_{local_rank}.pt")
