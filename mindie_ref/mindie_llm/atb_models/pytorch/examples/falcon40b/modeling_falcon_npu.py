...
class FalconAttention(nn.Module):
    ...
    def _split_heads(self, fused_qkv: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.new_decoder_architecture:
            ...
            # [+] fused_qkv shape: torch.Size([1, 32, 2304])
            # print(f"[+] fused_qkv shape: {fused_qkv.shape}")
            qkv = fused_qkv.view(batch, seq_len, -1, self.num_heads // self.num_kv_heads + 2, self.head_dim)
            # [+] qkv shape: torch.Size([1, 32, 2, 18, 64])
            query = qkv[:, :, :, :-2]
            key   = qkv[:, :, :, [-2]]
            value = qkv[:, :, :, [-1]]
            # RuntimeError: The expanded size of the tensor (16) must match the existing size (2) at non-singleton dimension 3.  
            # Target sizes: [1, 32, 2, 16, 64].  Tensor sizes: [1, 32, 2, 64]
            # key   = qkv[:, :, :, -2].view(batch, seq_len, qkv.shape[2], 1, self.head_dim)
            # value = qkv[:, :, :, -1].view(batch, seq_len, qkv.shape[2], 1, self.head_dim)
            key   = torch.broadcast_to(key, query.shape)
            value = torch.broadcast_to(value, query.shape)

            query, key, value = [x.flatten(2, 3) for x in (query, key, value)]
            # [+] shape: torch.Size([1, 32, 32, 64]) torch.Size([1, 32, 32, 64]) torch.Size([1, 32, 32, 64])
    ...
    def forward(
        ...
        **kwargs,
        ...
        # torch.save(fused_qkv, f"results/Attention_fused_qkv_{local_rank}.pt")
        num_kv_heads = self.num_heads if self.new_decoder_architecture else self.num_kv_heads
        # 3 x [batch_size, seq_length, num_heads, head_dim]
        (query_layer, key_layer, value_layer) = self._split_heads(fused_qkv)
        # torch.save(query_layer, f"results/Attention_query_layer_{local_rank}.pt")
        # torch.save(key_layer, f"results/Attention_key_layer_{local_rank}.pt")
        # [+] shape: torch.Size([1, 32, 24, 64]) torch.Size([1, 32, 24, 64]) torch.Size([1, 32, 24, 64])
        # print("[+] shape:", query_layer.shape, key_layer.shape, value_layer.shape)
        batch_size, query_length, _, _ = query_layer.shape
        # 1*32  32  64 num_heads/4=32, query_length=32, head_dim=64
        # 32*32*64 = 65536, but query_layer is 49152, 65536/4 * 3 = 49152  16384
        # print(f"[+] {(batch_size * self.num_heads, query_length, self.head_dim)}")
        ...
        # torch.save(query_layer, f"results/Attention_query_layer_emb_{local_rank}.pt")
        # torch.save(key_layer, f"results/Attention_key_layer_emb_{local_rank}.pt")
        ...
        if alibi is None:
            ...
            # torch.save(attn_output, f"results/Attention_attn_output_{local_rank}.pt")
            # if local_rank == 0:
            #     # print("[+] attn_output:", attn_output.min(), attn_output.max())
            #     torch.save(attn_output, f"results/attn_output_{self.world_size}_{local_rank}.pt")
            output_tensor = self.dense(attn_output)
            # torch.save(output_tensor, f"results/Attention_output_tensor_{local_rank}.pt")
            if self.world_size >= 2:
                torch.distributed.all_reduce(output_tensor, op=torch.distributed.ReduceOp.SUM)
            # torch.save(output_tensor, f"results/Attention_output_tensor_after_reduce_{local_rank}.pt")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.new_decoder_architecture:
            ...
            # local_rank = torch.distributed.get_rank()
            # torch.save(attention_layernorm_out, f"results/DecoderLayer_attention_layernorm_out_{local_rank}.pt")
            # torch.save(mlp_layernorm_out, f"results/DecoderLayer_mlp_layernorm_out_{local_rank}.pt")
        else:
            attention_layernorm_out = self.input_layernorm(hidden_states)
        
        # print("[+] attention_layernorm_out:", attention_layernorm_out.min(), attention_layernorm_out.max())
        # print("[+] mlp_layernorm_out:", mlp_layernorm_out.min(), mlp_layernorm_out.max())
        ...
        # torch.save(mlp_output, f"results/DecoderLayer_mlp_output_{local_rank}.pt")
...
class FalconModel(FalconPreTrainedModel):
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            local_rank = torch.distributed.get_rank()
            # torch.save(hidden_states, f"results/hidden_states_1205_{i}_{local_rank}.pt")
...
class FalconForCausalLM(FalconPreTrainedModel):
    ...
    def __init__(self, config: FalconConfig):
        ...
        self.inputs_acl = [None] * (10 + self.hidden_layers)
        ...
        print("[+] Init falcon model success")
        self.falcon_model.set_param(self.param)
        print("[+] Set falcon model param success")
    ...
    def forward(
        ...
        if self.acc_test:
            ...
            lm_logits_npu = self.lm_head(hidden_states)
        ...
        if past_key_values:
            ...
            if self.cache_k is None or batch_size != self.cache_k.shape[1] or self.total_seqlen != self.cache_k.shape[-2]:
                self.cache_k = torch.zeros(self.hidden_layers, batch_size, self.total_seqlen, self.head_dim*self.num_heads, dtype=torch.float16, device=device)
                self.cache_v = torch.zeros(self.hidden_layers, batch_size, self.total_seqlen, self.head_dim*self.num_heads, dtype=torch.float16, device=device)
                # self.rope.cos_sin(0, self.total_seqlen, position_id, device=device, dtype=torch.float16)
        ...
        rand_tensor = torch.rand((1, 1), device=device, dtype=torch.float16)
        ...
        #     with torch.npu.profile("./profile1128/", use_e2e_profiler=True, config=profile_config) as prof:
        #         torch.npu.synchronize()
        #         outputs_acl = self.falcon_model.execute(self.inputs_acl, run_param)
        #         torch.npu.synchronize()
        #         stream = torch.npu.current_stream()
        #         stream.synchronize()
        #     exit(0)
        ...
        # print(f"[+] falcon model execute time: {1000*(time.time()-acl_start_time):.4f}ms")
        lm_logits = out[0]
        # hidden_states = outputs_acl[0]
        # lm_logits = outputs_acl[1]
        if self.acc_test:
            # print(f"abs_sim: {torch.sum((lm_logits.to(torch.float64) - lm_logits_npu.to(torch.float64)).abs()) / lm_logits.numel()}")
            # print(f"max_sim: {torch.max((lm_logits.to(torch.float64) - lm_logits_npu.to(torch.float64)).abs())}")
            cos = torch.nn.CosineSimilarity(dim=0)
            cos_output = cos(lm_logits.view(-1).to(torch.float64), lm_logits_npu.view(-1).to(torch.float64))
            print(f"[+] Cosine Similarity: {cos_output}")
            assert cos_output > 0.99999
            # assert torch.allclose(lm_logits, lm_logits_npu, rtol=0.02, atol=0.02)
            # lm_logits = lm_logits_to_comp
            # hidden_states = hidden_states_to_comp
            # presents = transformer_outputs.past_key_values
