    ...
    def forward(self, input_ids, position_ids, attention_mask, *,
        ...
        if not hasattr(self, "kv_cache") or batch_size != self.batch_num:
            ...
            self.attention_mask_max = torch.full(
                ...
                0, dtype=torch.half, device=self.kv_cache.device)
        ...
        if query_length > 1:
            ...
            mask_temp = torch.ones(
                (batch_size, self.max_sequence_length, self.max_sequence_length), device=self.kv_cache.device)
            mask_temp.tril_()
            for i, context_length in enumerate(context_lengths):
                mask_temp[i, :, :context_length] = 1
