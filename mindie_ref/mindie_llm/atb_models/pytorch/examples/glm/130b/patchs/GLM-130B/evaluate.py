...
def add_evaluation_specific_args(parser):
    ...
    group.add_argument(
        "--task", nargs="+",
        help="All task config to evaluation"
    )
    group.add_argument(
        "--data-path", type=str,
        default=f"{os.environ.get('ATB_TESTDATA', '/data')}/evaluation",
        help="Data dir path for all tasks"
    )
...
def evaluate_all_tasks(data_path, model, tokenizer, all_task_config_path, 
                       task_classes):
...
def evaluate(model, tokenizer, task, data_path):
    task_ = find_all_tasks(task)

    task_classes = []
    print_rank_0("> Loading task configs")
    for task_config_path in task_:
        ...
        print_rank_0(
            f"    Task {config.name} loaded from config {task_config_path}")
    print_rank_0(f"> Successfully load {len(task_classes)} " \
                 f"task{'s' if len(task_classes) > 1 else ''}")

    model = ModelForEvaluation(model)

    start = time.time()
    evaluate_all_tasks(data_path, model, tokenizer,
                       task_, task_classes)
    print_rank_0(
        f"Finish {len(task_classes)} task{'s' if len(task_classes) > 1 else ''}" \
