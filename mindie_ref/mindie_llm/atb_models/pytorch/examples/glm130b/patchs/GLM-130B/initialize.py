...
def add_bminf_args(parser):
    ...
    group.add_argument("--bminf", action="store_true",
                       help="Use BMInf to support low resource evaluation")
    group.add_argument("--bminf-memory-limit", type=int,
                       default=20, help="Max memory for model per GPU (in GB)")
    return parser
...
def add_quantization_args(parser):
    ...
    group.add_argument("--from-quantized-checkpoint", action="store_true",
...
def add_atb_args(parser):
    group = parser.add_argument_group("ATB")
    group.add_argument(
        "--atb_backend",
        type=str,
        default="hccl",
        choices=["hccl", "lccl"],
        help="Set collective communications library for ATB"
    )    
...
def initialize(*extra_args_provider):
    ...
    for provider in extra_args_provider:
        provider(parser)
