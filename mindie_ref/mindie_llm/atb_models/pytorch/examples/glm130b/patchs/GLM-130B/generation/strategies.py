    ...
    def forward(self, logits, tokens, mems, temperature=None):
        ...
        for i in range(self.batch_size):
            if i >= batch_size:
                ...
                self._is_done[i] = True
        tokens = torch.cat(
            (tokens, pred.view(tokens.shape[:-1] + (1,))), dim=-1)
        return tokens, mems
    ...
    def forward(self, logits, tokens, mems):
        ...
        if self.deterministic:
            next_tokens = torch.topk(
                probs, k=(max(1, len(self.end_tokens)) + 1) * self.num_beams).indices  # [2*nb]
        else:
            next_tokens = torch.multinomial(
                probs, num_samples=(
                    max(1, len(self.end_tokens)) + 1) * self.num_beams
            )  # [2*nb]
        next_token_scores = next_token_scores[torch.arange(
            batch_size).unsqueeze(1), next_tokens]
        next_token_scores, _indices = torch.sort(
            next_token_scores, descending=True, dim=1)
        next_tokens = next_tokens[torch.arange(
            batch_size).unsqueeze(1), _indices]
        ...
        for batch_idx in range(batch_size):
            ...
            for i in range(len(next_tokens[batch_idx])):
                beam = torch.cat(
                    (tokens[batch_idx, next_indices[batch_idx, i]], next_tokens[batch_idx, i: i + 1]))
                if not self._is_done[batch_idx] and int(next_tokens[batch_idx, i]) in self.end_tokens:
                    self._add_end_beams(
                        next_token_scores[batch_idx, i], beam, batch_idx)
                elif len(beam_continue) < self.num_beams:
                    beam_continue.append(beam)
                    # mems_contiue.append(
                    #     mems[:, batch_idx, next_indices[batch_idx, i]])
                    # update caches
                    ...
                    if self.ngram > 0:
                        ...
                        bans[ngram_prefix] = bans.get(
                            ngram_prefix, tuple()) + (next_tokens[batch_idx, i],)
        ...
        if self.consider_end:
            ...
            for batch_idx in range(batch_size):
                if not self._is_done[batch_idx]:
                    for i in range(num_beams):
                        self._add_end_beams(
                            self.cached_beam_scores[batch_idx, i], tokens[batch_idx, i], batch_idx)
            mems = None
