...
class KVAttentionManager:
    def __init__(self, config: LlamaConfig, batch_size):
        self.is_full = True
        self.batch_size = batch_size
        self.num_layers = config.num_hidden_layers
        self.num_heads = config.num_attention_heads
        self.headSize = config.hidden_size // self.num_heads
        self.hidden_size = config.hidden_size // config.world_size
        self.kv_head_num = config.num_key_value_heads // config.world_size
        self.max_sequence_length = config.max_position_embeddings

        self.k_cache_input = torch.zeros(self.num_layers,
                                         batch_size,  # batch
                                         self.max_sequence_length,
                                         self.kv_head_num * self.headSize,
                                         device="cpu").half().npu()
        ...
        if self.soc_info.need_nz:
            self.k_cache_input = [torch_npu.npu_format_cast_(k_cache, 29) for k_cache in self.k_cache_input]
            self.v_cache_input = [torch_npu.npu_format_cast_(v_cache, 29) for v_cache in self.v_cache_input]
        self.token_offset = 1
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def init_ascend_operations(self, config: LlamaConfig):
        ...
        if hasattr(config, 'world_size'):
            ...
            self.acl_param = json.dumps({"headNum": self.head_num, "rmsNormEps": config.rms_norm_eps, "kvHeadNum": self.kv_head_num,
        ...
        self.acl_fa_operation = torch.classes.ModelTorch.ModelTorch("codellama_34b_flash_attention_rope_model")
        self.acl_fa_operation.set_param(self.acl_param)
        self.ascend_rotary_embedding = AscendRotaryEmbedding(
            config.hidden_size // config.num_attention_heads, max_position_embeddings=self.max_position_embeddings, base=config.rope_theta)
    ...
    def init_ascend_weight(self):
        ...
        for i in range(self.num_layers):
            ...
            weights_t.append(self.weight_format_cast(weights_layer["self_attn.q_proj.weight"]))
            weights_t.append(self.weight_format_cast(weights_layer["self_attn.k_proj.weight"]))
            weights_t.append(self.weight_format_cast(weights_layer["self_attn.v_proj.weight"]))
            weights_t.append(self.weight_format_cast(weights_layer["self_attn.o_proj.weight"]))
            weights_t.append(weights_layer["post_attention_layernorm.weight"])
            weights_t.append(self.weight_format_cast(weights_layer["mlp.gate_proj.weight"]))
            weights_t.append(self.weight_format_cast(weights_layer["mlp.down_proj.weight"]))
            weights_t.append(self.weight_format_cast(weights_layer["mlp.up_proj.weight"]))
