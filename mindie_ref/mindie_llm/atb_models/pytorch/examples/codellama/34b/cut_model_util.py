...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.input_path, torch_dtype=torch.float16
    )  # load the raw model
    state_dict_list = cut_weights(
        model, args.world_size, args.cut_row_keys, args.cut_col_keys
    )  # cut the weight
    ...
    for i in range(args.world_size):
        ...
        creat_model.save_pretrained(
            args.output_path + "/part_model/" + str(i) + "/"
        )  # save model
