...
time_now = time.strftime("%H-%M-%S--%Y-%d-%m", time.localtime())
file_name = f"model_compare_gptneox_20b_{time_now}.csv"
with open(file_name, 'a') as file:
    file.write(f"CompareType,TokenNumber,LayerId,Origin VS ACL Model\n")
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def init_ascend_operations(self, config: GPTNeoXConfig):
        ...
        self.attention_mask_generator = AttentionMask.static(self.max_position_embeddings)
        self.attention_mask_decoder = torch.full((self.max_position_embeddings, self.max_position_embeddings), 0,
                                                 dtype=torch.half).npu()
        ...
        self.acl_encoder_operation_inputs = [None] * (9 + self.num_layers)
        ...
        for i in range(self.num_layers):
            self.acl_encoder_operation_inputs[9 + i] = torch.tensor([i], dtype=torch.int32).npu()
    ...
    def prepare_inputs_for_ascend(self, input_ids, position_ids, seq_length, batch_size, past_key_values=None):
        ...
        if not past_key_values or past_key_values[0] is None:
            ...
            self.attention_mask_encoder = self.attention_mask_generator.get_attn_mask(seq_length,
    ...
    def forward(
        ...
        # obtain ACL hidden states result
        ...
        acl_hidden_states = self.execute_ascend_operator(model_op, input_ids, position_ids, past_key_values)
        # presents = (self.k_cache_input, self.v_cache_input)

        # obtain Pytorch hidden states result
        pytorch_all_hidden_states = ()
        pytorch_all_attentions = ()
        for i, (layer, layer_past) in enumerate(zip(self.layers, past_key_values)):
            if output_hidden_states:
                pytorch_all_hidden_states = pytorch_all_hidden_states + (hidden_states,)
            ...
            if output_attentions:
                pytorch_all_attentions = pytorch_all_attentions + (outputs[2 if use_cache else 1],)
        hidden_states = self.final_layer_norm(hidden_states)

        # with open(file_name, 'a') as file:
        #     for i in range(len(presents)):
        #         test_key = presents[i][0].permute(0, 2, 1, 3).reshape(-1, self.token_num, self.hidden_size)
        #         test_value = presents[i][1].permute(0, 2, 1, 3).reshape(-1, self.token_num, self.hidden_size)
        #         acl_key = self.k_cache_input[i][:, :self.token_num, :]
        #         acl_value = self.v_cache_input[i][:, :self.token_num, :]
        #         is_key_equal = np.allclose(acl_key.cpu(), test_key.cpu(), rtol=0.02, atol=0.02)
        #         is_value_equal = np.allclose(acl_value.cpu(), test_value.cpu(), rtol=0.02, atol=0.02)
        #         file.write(f"cache_key,{self.token_num},{i},{is_key_equal}\n")
        #         file.write(f"cache_v,{self.token_num},{i},{is_value_equal}\n")

        if output_hidden_states:
            all_hidden_states = all_hidden_states + (acl_hidden_states,)

        if not return_dict:
            return tuple(v for v in [acl_hidden_states, presents, all_hidden_states, all_attentions,
                                     hidden_states] if v is not None)

        # Add last hidden state
        return (BaseModelOutputWithPast(
            last_hidden_state=acl_hidden_states,
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.position_ids_cache = torch.arange(config.max_position_embeddings, device='cpu').long().unsqueeze(0).npu()
        self.lm_head_weight = None
    ...
    def forward(
        ...
        )  # (BaseModelOutputWithPast, pytorch_hidden_states)

        # ACL and Pytorch output
        if not return_dict:
            ...
            acl_lm_logits = model_output.last_hidden_state
            pytorch_hidden_states = outputs[1]
        else:
            model_output = outputs[0]
            acl_lm_logits = model_output[0]
            pytorch_hidden_states = outputs[1]
        lm_logits = self.embed_out(pytorch_hidden_states)

        with open(file_name, 'a') as file:
            is_output_equal = np.allclose(acl_lm_logits.cpu(), lm_logits.cpu(), rtol=0.02, atol=0.02)
            file.write(f"output_logits,{acl_lm_logits.shape[1]}, ,{is_output_equal}\n")

        lm_loss = None
        if labels is not None:
            # move labels to correct device to enable model parallelism
            labels = labels.to(acl_lm_logits.device)
            # we are doing next-token prediction; shift prediction scores and input ids by one
            shift_logits = acl_lm_logits[:, :-1, :].contiguous()
            labels = labels[:, 1:].contiguous()
            loss_fct = CrossEntropyLoss()
            lm_loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), labels.view(-1))

        if not return_dict:
            output = (acl_lm_logits,) + model_output[1:]
