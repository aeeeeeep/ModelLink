...
# rotary_dims = hidden_size/num_attention_heads * rotary_pct => 24
...
emb = torch.cat((freqs, freqs), dim=-1)
cosTable = emb.cos().half().npu()  # [max_sq, rotary_dims]
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        acl_param_fa = json.dumps({
            ...
            "operationNum": self.num_layers + 2})
    ...
    def forward(
        ...
        if batch_size != self.batch_num or self.kv_cache is None:
        ...
        if self.weight_flag is False:
            ...
            # print("==all model state_dict is: ")
            # for key, value in self.state_dict().items():
            #     print("model weight key: ", key, "value shape: ", value.shape)
        ...
        if is_prefill:
            self.token_num = 0
            self.seq_len_tensor = torch.tensor([seq_length] * batch_size, dtype=torch.int32, device=input_ids.device)
