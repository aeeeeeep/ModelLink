...
class SelfAttention(torch.nn.Module):
    def __init__(self, config, hidden_size, num_attention_heads,
        ...
        self.query_key_value = skip_init(
            torch.nn.Linear,
            hidden_size,
            3 * self.inner_hidden_size // self.world_size,
            bias=bias,
            dtype=params_dtype,
        )
        ...
        self.dense = skip_init(
            torch.nn.Linear,
            self.inner_hidden_size // self.world_size,
            hidden_size,
            bias=bias,
            dtype=params_dtype,
        )
    ...
    def forward(
        ...
        new_tensor_shape = mixed_raw_layer.size()[:-1] + (
            self.num_attention_heads_per_partition // self.world_size,
        ...
        context_layer, present, attention_probs = attention_fn(
            ...
            hidden_size_per_partition=self.hidden_size_per_partition // self.world_size,
            layer_id=layer_id,
...
class GLU(torch.nn.Module):
    def __init__(self, config, hidden_size, inner_hidden_size=None,
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    def __init__(self, config):
        ...
        if soc_version in [104, 220, 221, 222, 223, 224]:
            ...
            self.format_nz = True
            self.hidden_dim_nz = math.ceil(config.hidden_size // self.world_size / NZ_LAST_DIM)

        # init required inputs
        self.num_layers = config.num_layers
        self.model_rotary_pos_emb_cos = None, 
        self.model_rotary_pos_emb_sin = None,
        ...
        self.manager = ModeManager.get_manager(mode=self.mode)(self.config, rank=self.rank, world_size=self.world_size, device="npu")
    ...
    def forward(
        ...
        if full_flag:
            # obtain attention_mask and position_id
            seq = input_ids[0].tolist()

            if attention_mask is None:
                attention_mask = self.transformer.get_masks(
                    seq=seq,
                    device=input_ids.device
                )
            ...
            if position_ids is None:
                ...
                mask_position = seq.index(mask_token)
                position_ids = self.transformer.get_position_ids(
                    seq=seq,
                    mask_position=mask_position,
                    device=input_ids.device,
                    gmask=use_gmask
        ...
        if attention_mask is None:
            ...
            attention_mask_acl = attention_mask
