    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        if torch.distributed.get_rank() == 0:
            print ("-----------------------------------------------Time analysis-----------------------------------------")
            print("single token time", time_list, "\n")
            print("total time", sum(time_list), "\n")
            print("E2E time", E2E_end_time - E2E_time_start, "\n")
            save_path="mixtral_8x7b_performance"
            name=save_path+"/time"
            if not os.path.exists(name):
                os.makedirs(name)            
            np.savetxt(name+"/single_token_time.txt",np.array(time_list))
            np.savetxt(name+"/total_time.txt",np.array([sum(time_list)]))
            np.savetxt(name+"/E2E_time.txt",np.array([E2E_end_time - E2E_time_start]))
