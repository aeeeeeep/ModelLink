...
# 权重路径，需适配不同机器进行修改
weight_dir="/home/data/acltransformer_testdata/weights/Mixtral-8x7B-v0.1-part-model-8"
...
# MODELING_SCRIPT_PATH="$ATB_SPEED_HOME_PATH/pytorch/examples/mixtral_8x7B_densed/patches/layer/modeling_mixtral.py"
MODELING_SCRIPT_PATH="$ATB_SPEED_HOME_PATH/pytorch/examples/mixtral_8x7B_densed/patches/layer/modeling_mixtral_atb.py"

# 性能打点文件侵入式修改
UTILS_PATH="$TRANSFORMER_PACKAGE_PATH/generation/utils.py"
PERSONAL_UTILS_PATH="$ATB_SPEED_HOME_PATH/pytorch/examples/mixtral_8x7B_densed/utils.py"

# 模型配置文件修改
CONFIG_PATH="$ATB_SPEED_HOME_PATH/pytorch/examples/mixtral_8x7B_densed/config.json"
TARGET_JSON_0_PATH="$weight_dir/part_model/0/config.json"
TARGET_JSON_1_PATH="$weight_dir/part_model/1/config.json"
TARGET_JSON_2_PATH="$weight_dir/part_model/2/config.json"
TARGET_JSON_3_PATH="$weight_dir/part_model/3/config.json"
TARGET_JSON_4_PATH="$weight_dir/part_model/4/config.json"
TARGET_JSON_5_PATH="$weight_dir/part_model/5/config.json"
TARGET_JSON_6_PATH="$weight_dir/part_model/6/config.json"
TARGET_JSON_7_PATH="$weight_dir/part_model/7/config.json"

cp $CONFIG_PATH $TARGET_JSON_0_PATH
cp $CONFIG_PATH $TARGET_JSON_1_PATH
cp $CONFIG_PATH $TARGET_JSON_2_PATH
cp $CONFIG_PATH $TARGET_JSON_3_PATH
cp $CONFIG_PATH $TARGET_JSON_4_PATH
cp $CONFIG_PATH $TARGET_JSON_5_PATH
cp $CONFIG_PATH $TARGET_JSON_6_PATH
cp $CONFIG_PATH $TARGET_JSON_7_PATH
cp $PERSONAL_UTILS_PATH $UTILS_PATH
cp $MODELING_SCRIPT_PATH $TRANSFORMER_PACKAGE_PATH/models/mixtral/modeling_mixtral.py
