...
class BaichuanLMParallelQuant(ParallelLauncher):
    """
    多卡推理launcher
    """
    ...
    def init_model(self):
        ...
        config = BaichuanConfig.from_pretrained(part_model_path, trust_remote_code=True)
        config._name_or_path = part_model_path
        model = AutoModelForCausalLM.from_config(config, trust_remote_code=True).half().npu()
        model.eval()
        model.generation_config = self.remove_part_of_generation_config(model.generation_config)
        return model, tokenizer
...
class BaichuanLMQuant(Launcher):
    """
    多卡推理launcher
    """

    def init_model(self):
        tokenizer = AutoTokenizer.from_pretrained(self.model_path, trust_remote_code=True, local_files_only=True,
                                                  use_fast=False, padding_side='left')

        config = BaichuanConfig.from_pretrained(self.model_path, trust_remote_code=True)
