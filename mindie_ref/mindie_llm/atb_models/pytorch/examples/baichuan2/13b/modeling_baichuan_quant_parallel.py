...
def record_npu_memory(name):
    print_rank_0(
        f"[{name.center(20, '*')}] [reserved:{torch.npu.memory_reserved() / 1024 / 1024:.0f}M] [allocated: {torch.npu.memory_allocated() / 1024 / 1024:.0f}M]")
...
class BaichuanModel(BaichuanPreTrainedModel):
    def __init__(self, config: BaichuanConfig):
        ...
        self.quant_weight_path = "/home/hyxxxx/baichuan/quant_model_path/data_1212_pr1_cut_int/"
        self.roll_back_layer = [0, 1, 2, 3, 4, 7, 9, 10, 17, 18, 19, 20, 22, 23, 24, 26, 36, 37, 38, 39]
        self.is_opera = False

        # self.quant_weight_path = '/home/ctl/models/data_1221_0_10_39_cut_int'
        # self.roll_back_layer = [0,10,39]
        # self.is_opera = True

        self.float_weight_path = "/home/hyxxxx/baichuan/model_path"

        if self.rankSize > 1:
            self.cut_float_weight = os.path.join(self.float_weight_path, 'part_model', str(self.rank))
            self.part_quant_weight_path = os.path.join(self.quant_weight_path, str(self.rank))
        else:
            self.cut_float_weight = self.float_weight_path
            self.part_quant_weight_path = self.quant_weight_path
    ...
    def process_deq_scale(self, deq_scale_tensor):
        """
        deq转成int64
        """
        new_deq_scale = np.frombuffer(deq_scale_tensor.numpy().tobytes(), dtype=np.uint32)
        return torch.from_numpy(new_deq_scale.astype(np.int64))
    ...
    def init_ascend_operations(self, config: BaichuanConfig):
        ...
        self.acl_param = json.dumps({
            ...
            "isOpera": self.is_opera
        })

        print(self.acl_param)
        self.max_position_embeddings = int(os.getenv("MAX_SEQ_LEN", config.model_max_length))
        self.acl_fa_operation = torch.classes.ModelTorch.ModelTorch("baichuan2_13b_flash_attention_quant_model")
    ...
    def init_ascend_weight(self):
        ...
        if self.rankSize > 1:
            ...
            quant_weight_dict = np.load(os.path.join(self.quant_weight_path, "quant_weight.npy"),
                                        allow_pickle=True).item()
            deq_scale_dict = np.load(os.path.join(self.quant_weight_path, "deq_scale.npy"),
                                     allow_pickle=True).item()
            quant_bias_dict = np.load(os.path.join(self.quant_weight_path, "bias.npy"),
                                      allow_pickle=True).item()
        ...
        for layer_index in range(self.num_layers):
            ...
            if layer_index in self.roll_back_layer:
                ...
                if not IS_ND:
                    ...
                    self.weights.append(self.model_weights.get(w_pack_name).to(torch.float16).npu())
                    self.weights.append(self.model_weights.get(o_proj_name).to(torch.float16).npu())
                    self.weights.append(self.model_weights.get(post_attention_layernorm_name).to(torch.float16).npu())
                    self.weights.append(self.model_weights.get(gate_proj_name).to(torch.float16).npu())
                    self.weights.append(self.model_weights.get(down_proj_name).to(torch.float16).npu())
                    self.weights.append(self.model_weights.get(up_proj_name).to(torch.float16).npu())
                ...
                gate_proj_name = "model.layers.{}.mlp.gate_proj".format(layer_index)
                down_proj_name = "model.layers.{}.mlp.down_proj".format(layer_index)
                down_proj_quant_name = "model.layers.{}.mlp.down_proj".format(layer_index)
                down_proj_float_name = "model.layers.{}.mlp.down_proj.weight".format(layer_index)
                # int8 weight
                if not IS_ND:
                    self.weights.append(
                    ...
                    if self.is_opera:
                        self.weights.append(torch_npu.npu_format_cast(
                            self.model_weights.get(down_proj_float_name).to(torch.float16).npu(), 29))
                        self.weights.append(deq_scale_dict[gate_proj_name].to(torch.int64).npu())
                        self.weights.append(quant_bias_dict[gate_proj_name].to(torch.int32).npu())
                    else:
                        self.weights.append(
                    ...
                    self.weights.append(quant_weight_dict[w_pack_name].to(torch.int8).npu())
                    self.weights.append(self.process_deq_scale(deq_scale_dict[w_pack_name]).npu())
                    self.weights.append(quant_bias_dict[w_pack_name].to(torch.int32).npu())

                    self.weights.append(quant_weight_dict[o_proj_name].to(torch.int8).npu())
                    self.weights.append(self.process_deq_scale(deq_scale_dict[o_proj_name]).npu())
                    self.weights.append(quant_bias_dict[o_proj_name].to(torch.int32).npu())

                    self.weights.append(quant_weight_dict[up_proj_name].to(torch.int8).npu())
                    self.weights.append(self.process_deq_scale(deq_scale_dict[up_proj_name]).npu())
                    self.weights.append(quant_bias_dict[up_proj_name].to(torch.int32).npu())

                    self.weights.append(quant_weight_dict[gate_proj_name].to(torch.int8).npu())
                    self.weights.append(self.process_deq_scale(deq_scale_dict[gate_proj_name]).npu())
                    self.weights.append(quant_bias_dict[gate_proj_name].to(torch.int32).npu())

                    if self.is_opera:
                        self.weights.append(self.model_weights.get(down_proj_float_name).to(torch.float16).npu())
                        self.weights.append(deq_scale_dict[gate_proj_name].to(torch.int64).npu())
                        self.weights.append(quant_bias_dict[gate_proj_name].to(torch.int32).npu())
                    else:
                        self.weights.append(quant_weight_dict[down_proj_name].to(torch.int8).npu())
                        self.weights.append(self.process_deq_scale(deq_scale_dict[down_proj_name]).npu())
                        self.weights.append(quant_bias_dict[down_proj_name].to(torch.int32).npu())
                    # rms norm float
                    self.weights.append(self.model_weights.get(post_attention_layernorm_name).to(torch.float16).npu())
        ...
        if not IS_ND:
            self.weights.append(
                torch_npu.npu_format_cast(self.model_weights['lm_head.weight'].to(torch.float16).npu(), 29))
        else:
            self.weights.append(self.model_weights['lm_head.weight'].to(torch.float16).npu())
    ...
    def get_alibi_mask(self, tensor, seq_length_with_past):
        if self.training:
            ...
            if self.rankSize > 1:
                mask = mask.chunk(self.rankSize, dim=0)
        return mask
    ...
    def forward(
        ...
        # print("attention mask shap ",attention_mask.shape)
