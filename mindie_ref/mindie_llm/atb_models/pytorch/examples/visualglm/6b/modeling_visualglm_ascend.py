...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        hidden_states_ = attention_input * alpha + attention_output

        mlp_input = self.post_attention_layernorm(hidden_states_)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig, empty_init=True):
        ...
        qScale = [(1 / (math.sqrt(self.hidden_size_per_attention_head) * float(layer_id + 1))) for layer_id in range(self.num_layers)]
    ...
    def get_masks(self, seq, device, padding_mask=None):
    ...
    def forward(
        ...
        if self.weightFlag is False:
            ...
            weights.append(self.config.lm_head.weight)
        ...
        if full_flag:
            ...
            self.token_offset = torch.tensor(self.token_num, dtype=torch.int32, device=input_ids.device)
            self.seq_len = torch.tensor(self.param_seq_len, dtype=torch.int32, device=input_ids.device)
            self.attention_mask_max_full[:seqlen, :seqlen] = 0
            self.attention_mask_max_full[:seqlen, :seqlen].masked_fill_(full_attention_mask.squeeze(), -10000)
            self.acl_operation_inputs[0] = hidden_states
            ...
            acl_param = json.dumps({"tokenOffset": self.token_num, "seqLen": self.param_seq_len})
            acl_model_out = self.acl_encoder_operation.execute(self.acl_operation_inputs, acl_param)
            
            self.attention_mask_max_full[:seqlen, :seqlen] = -10000.0
            self.param_seq_len = [1] * batch_size
            self.seq_len = torch.tensor(self.param_seq_len, dtype=torch.int32, device=self.kv_cache.device)
        else:
        ...
        if os.environ.get("PRECISION_TEST"):
            ...
            for i, layer in enumerate(self.layers):
                ...
                # torch.save(hidden_states.cpu(), f"tensor/layer_output.pth")
            ...
            if torch.allclose(hidden_states, acl_model_out[0], atol=0.03, rtol=0.03):
                print("equal")
            else:
...
class ChatGLMForConditionalGenerationWithImage(ChatGLMForConditionalGeneration):
    ...
    def build_inputs_with_image(self, tokenizer, image_path: str, query: str, history: List[Tuple[str, str]] = None):
        ...
        if torch_image is not None:
            ...
            inputs = {
                "input_ids": torch.tensor([tokenizer.build_inputs_with_special_tokens(inputs)], dtype=torch.long).to(self.device),
    ...
    def forward(
        ...
        if inputs_embeds is None and past_key_values is None and images is not None:
            ...
            image_embeds = torch.tensor(IMAGE_ENCODER_OM().image_encoder_om.infer(images)[0]).to(self.device)
            print('picture process time:', time.time() - time_start)
