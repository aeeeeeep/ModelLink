...
class KVAttentionManager:
    def __init__(self, num_hidden_layers,hidden_size,max_position_embeddings, batch_size, seq_length):
        self.is_full = True
    ...
    def get_attention_mask(self, attention_mask=None):
        # print(f"original attention_mask = {attention_mask}")
        # if attention_mask is not None:
        #     print(f"original attention_mask.size = {attention_mask.size()}")
...
class VLMo(pl.LightningModule):
    def __init__(self, config):
        ...
        self.relative_position_bias_list_vl = self.get_rel_pos_bias(self.text_imag_relative_position_index)
        self.relative_position_bias_list_text = self.get_rel_pos_bias(self.text_relative_position_index)
        self.relative_position_bias_list_image = self.get_rel_pos_bias(self.relative_position_index)
        # print("<><><><><>< self.relative_position_bias_list shape", self.relative_position_bias_list.shape)
        
        self.world_size = WORLD_SIZE
        self.rank = RANK
        self.init_ascend_operations(config)
        self.layer_id_list = [torch.tensor([i], dtype=torch.int32).npu() for i in range(self.transformer.depth)]
    ...
    def init_acl_weight(self,modality_type="vl"):
        ...
        empty_k_bias = torch.zeros(self.transformer.embed_dim,
                            device="npu",
                            dtype=torch.float16)
    ...
    def init_acl_encoder_param(self, x, mask=None,modality_type="vl"):
        ...
        maskBool = ~mask.to(dtype=torch.bool)
        if modality_type == "vl":
            inputs = [x, 
                    maskBool, 
    ...
    def infer(
        ...
        for i, blk in enumerate(self.transformer.blocks):
            ...
            x = blk(x, mask=co_masks, modality_type="vl", relative_position_bias=relative_position_bias)
    ...
    def infer_ascend(
        ...
        if batch_size != self.batch_size_vl:
            ...
            self.kv_attention_manager_vl = KVAttentionManager(self.transformer.num_heads,self.transformer.embed_dim,input_shape, batch_size, input_shape)
        self.kv_attention_manager_vl.init_seq_len_and_token_offset(input_shape)  # for ascend
    ...
    def infer_text_ft_ascend(
        ...
        if batch_size != self.batch_size_text:
            ...
            self.kv_attention_manager_text = KVAttentionManager(self.transformer.num_heads,self.transformer.embed_dim,input_shape, batch_size, input_shape)
        self.kv_attention_manager_text.init_seq_len_and_token_offset(input_shape)  # for ascend
    ...
    def infer_image_ft_ascend(
        ...
        if batch_size != self.batch_size_image :
            ...
            self.kv_attention_manager_image = KVAttentionManager(self.transformer.num_heads,self.transformer.embed_dim,input_shape, batch_size, input_shape)
        self.kv_attention_manager_image.init_seq_len_and_token_offset(input_shape)  # for ascend
