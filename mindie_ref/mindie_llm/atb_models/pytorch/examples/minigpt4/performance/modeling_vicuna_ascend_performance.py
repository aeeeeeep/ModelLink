...
# Quant weight path
QUANT_WEIGHT_PATH = "../../llama/llama7b_quant_weight"
# Anti-outlier quant weight path
ANTI_QUANT_WEIGHT_PATH = "../../llama/llama7b_quant_weight/anti_weight"

# Sparse weight path
COMPRESS_WEIGHT_PATH = "../../llama/llama7b_sparsequant_weight"
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_ascend(self, batch_size, seq_length, input_ids, inputs_embeds, position_ids, attention_mask,
                                  past_key_values):
        if self.batch_num != batch_size:
            ...
            if not self.is_triu_mask:
                self.attention_mask_max_incre = torch.zeros(
                    (self.batch_num, math.ceil(self.max_sequence_length / self.nz_dim), self.max_sequence_length,
                     self.nz_dim),
                    device='npu',
                    dtype=self.dtype
                ).contiguous()
        ...
        self.token_offset = torch.full((self.batch_num,), self.token_num, dtype=torch.int32,
        ...
        if past_key_values is None:  # 首轮
            inputs_embeds = torch.randn((input_ids.shape[0], input_ids.shape[1], self.hidden_size)).half().npu()
    ...
    def init_ascend_operations(self):
        ...
        self.acl_encoder_operation = torch.classes.ModelTorch.ModelTorch(
            "minigpt4_vicuna_7b_FlashAttentionEncoderModel")
        ...
        for i in range(self.num_layers):
            if not self.is_ascend_quant or (self.is_ascend_quant and i in self.float_layers):
                ...
                if self.quant_model and not self.format_nz:
                    ...
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[q_name].to(torch.int8).npu()])[0])
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[k_name].to(torch.int8).npu()])[0])
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[v_name].to(torch.int8).npu()])[0])
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[o_name].to(torch.int8).npu()])[0])
                    if self.anti_quant_model:
                        weights_p.append(self.anti_quant_weight_dict.get(post_norm_weight).to(self.dtype).npu())
                    else:
                        weights_p.append(weights.get("post_attention_layernorm.weight"))
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[gate_name].to(torch.int8).npu()])[0])
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[down_name].to(torch.int8).npu()])[0])
                    weights_p.append(
                        self.transdata_operation.execute([self.quant_weight_dict[up_name].to(torch.int8).npu()])[0])
    ...
    def update_ascend_mask(self, attention_mask, seq_length):
        if self.is_triu_mask:
            if self.full_flag:
                self.attention_mask_max = self.get_bf16_mask(self.get_triumask(self.mask_block_size)).npu().to(
                    dtype=self.dtype)
                self.attention_mask_max_incre = torch.zeros((self.batch_num, 1, self.max_sequence_length),
                                                            dtype=self.dtype).npu()
                if self.format_nz:
                    soc_version = torch_npu._C._npu_get_soc_version()
        ...
        if self.full_flag:
            ...
            if attention_mask is not None:
                ...
                self.attention_mask_max[:self.batch_num, :seq_length,
                :attention_mask_acl.size()[-1]] += attention_mask_acl
            if self.format_nz:
                self.attention_mask_max = torch_npu.npu_format_cast(
                    self.attention_mask_max.view(self.batch_num, self.max_sequence_length,
                                                 self.max_sequence_length // self.nz_dim, self.nz_dim).transpose(1,
                                                                                                                 2).contiguous(),
                    29)
            ...
            if not self.decoder_mask:
                ...
                if self.format_nz:
                    ...
                    attention_mask_acl = attention_mask_acl.unsqueeze(-2).repeat(1, self.nz_dim, 1).view(self.batch_num,
                                                                                                         self.nz_dim,
                                                                                                         math.ceil(
                                                                                                             self.max_sequence_length / self.nz_dim),
                                                                                                         self.nz_dim).transpose(
                        1, 2)
                    ...
                    self.attention_mask_max_incre[:self.batch_num, :1,
                    :attention_mask_acl.size()[-1]] += attention_mask_acl.unsqueeze(-2)
    ...
    def load_ascend_quant_weight(self):
        ...
        if self.quant_model:
            ...
            self.input_offset_dict = np.load(os.path.join(self.quant_weight_path, "input_offset.npy"),
                                             allow_pickle=True).item()
            # load quant weight
            self.quant_weight_dict = np.load(os.path.join(self.quant_weight_path, "quant_weight.npy"),
                                             allow_pickle=True).item()
            self.deq_scale_dict = np.load(os.path.join(self.quant_weight_path, "deq_scale.npy"),
                                          allow_pickle=True).item()
            if self.world_size > 1:
                self.quant_bias_dict = np.load(os.path.join(self.quant_weight_path, "bias.npy"),
                                               allow_pickle=True).item()
            else:
                ...
                for i in fp_bias_dict.keys():
                    self.quant_bias_dict[i] = bias_correction(fp_bias_dict[i], self.quant_weight_dict[i],
                                                              int(self.input_offset_dict[i]), self.deq_scale_dict[i])
            ...
            if self.anti_quant_model:
                ...
                for weight_file in weight_files:
                    self.anti_quant_weight_dict.update(
                        torch.load(os.path.join(self.anti_quant_weight_path, weight_file), map_location='cpu'))
    ...
    def forward(
        ...
        # set ascend inputs
        self.prepare_inputs_for_ascend(batch_size, seq_length, input_ids, inputs_embeds, position_ids, attention_mask,
                                       past_key_values)
