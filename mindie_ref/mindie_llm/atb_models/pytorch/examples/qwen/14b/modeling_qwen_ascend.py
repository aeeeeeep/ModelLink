...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.c_attn = nn.Linear(config.hidden_size, 3 * self.num_heads * self.head_dim)

        self.c_proj = nn.Linear(
            self.num_heads * self.head_dim, self.projection_size, bias=not config.no_bias
        )
    ...
    def forward(
        ...
        # [batchSize, seqLen, hiddenSize] -> [batchSize, seqLen, headNum, headDim]
        ...
        if self.use_logn_attn and not self.training:
            if self.use_cache_quantization:
                ...
                seq_end = key.size(1)
            logn_tensor = self.logn_tensor[:, seq_start:seq_end, :, :].type_as(query)  # cast to the dtype of query
...
class QWenMLP(nn.Module):
    def __init__(self, config):
        ...
        # up_proj
        self.w1 = nn.Linear(
            config.hidden_size, rank_intermediate_size // 2, bias=not config.no_bias
        )
        # gate_proj
        self.w2 = nn.Linear(
            config.hidden_size, rank_intermediate_size // 2, bias=not config.no_bias
        )
        ff_dim_in = rank_intermediate_size // 2
        # down_proj
    ...
    def get_attention_mask(self, attention_mask=None):
        if not self.is_full:
            ...
            if attention_mask is not None:
                attention_mask = attention_mask.expand(
                    -1, -1, causal_mask.size(2), -1
                ).masked_fill(~causal_mask, torch.finfo(attention_mask.dtype).min)
            else:
                attention_mask = causal_mask
...
class QWenModel(QWenPreTrainedModel):
    ...
    def init_ascend_operations(self, config):
        self.head_size_ori = config.hidden_size // config.num_attention_heads
        param_dict = {
            "headNum": config.num_attention_heads // WORLD_SIZE,
            "RmsNormEps": config.layer_norm_epsilon,
            "dk": self.head_size_ori,
...
>>>>>>> a0b6776 (解决蓝区qwen浮点加速库无法跑起问题)
    ...
    def init_ascend_weight(self):
        ...
        for i in range(self.num_layers):
            ...
            weights_t.append(weights_layer["attn.c_attn.weight"])  # IN_QKVMIXEDLINEARWEIGHT
            weights_t.append(weights_layer["attn.c_attn.bias"])  # IN_QKVMIXEDLINEARBIAS
            weights_t.append(weights_layer["attn.c_proj.weight"])  # IN_SELFOUTLINEARWEIGHT
            weights_t.append(weights_layer["ln_2.weight"])  # IN_SELFOUTNORMWEIGHT
            weights_t.append(weights_layer["mlp.w1.weight"])  # IN_MLPW1WEIGHT
            weights_t.append(weights_layer["mlp.w2.weight"])  # IN_MLPW2WEIGHT
            weights_t.append(weights_layer["mlp.c_proj.weight"])  # IN_MLPCPROJWEIGHT
        ...
        self.acl_operation_encoder.set_weight(weights)
        self.acl_operation_decoder.set_weight(weights)

    def prepare_inputs_for_ascend(self, input_ids, rotary_pos_emb_list, attention_mask=None, past_key_values=None):
        self.kv_attention_manager.is_full = False if not past_key_values else True
        rotary_pos_emb = rotary_pos_emb_list[0]
        cos_embed = rotary_pos_emb[0][:, -input_ids.shape[1]:, :, :]
        sin_embed = rotary_pos_emb[1][:, -input_ids.shape[1]:, :, :]
        cos_embed = cos_embed.repeat(input_ids.shape[0], 1, 1, 1)
        sin_embed = sin_embed.repeat(input_ids.shape[0], 1, 1, 1)

        seq_start = self.kv_attention_manager.token_offset - input_ids.shape[-1]
    ...
    def execute_ascend_operator(self, input_ids, rotary_pos_emb_list, attention_mask=None, past_key_values=None):
        acl_inputs = self.prepare_inputs_for_ascend(input_ids, rotary_pos_emb_list, attention_mask, past_key_values)
        tmp_param = json.dumps(
            {"tokenOffset": self.kv_attention_manager.token_offset_list,
             "seqLen": self.kv_attention_manager.seq_len_list
             })
        if self.kv_attention_manager.is_full:
            acl_model_out = self.acl_operation_encoder.execute(acl_inputs, tmp_param)
        else:
            acl_model_out = self.acl_operation_decoder.execute(acl_inputs, tmp_param)
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            self.kv_attention_manager.init_seq_len_and_token_offset(input_shape[-1])  # for ascend
            ...
            if self.use_cache_quantization:
                ...
                # for ascend
                past_length = self.kv_attention_manager.token_offset
                # seq_length_with_past = input_shape[-1] + past_length
        ...
        if attention_mask is not None:
            self.kv_attention_manager.ori_len_list = attention_mask.sum(dim=-1)  # for ascend
            if batch_size <= 0:
        ...
        kv_seq_len = self.kv_attention_manager.token_offset
        if self.training or not self.use_dynamic_ntk:
            ...
            if attention_mask is not None and kv_seq_len > self.seq_length:
                # true_seq_lens_ = attention_mask.squeeze(1).squeeze(1).eq(0).sum(dim=-1, dtype=torch.int32)
                true_seq_lens = self.kv_attention_manager.ori_len_list
        ...
        rotary_pos_emb_list = [
            self.rotary_emb(self.kv_attention_manager.token_offset, ntk_alpha=ntk_alpha) for ntk_alpha in ntk_alpha_list
        ]
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if self.transformer.lm_head_weight is None:
            if not IS_ND:
                self.transformer.lm_head_weight = torch_npu.npu_format_cast(self.lm_head.weight.data, 29)
            self.transformer.lm_head_weight = self.lm_head.weight.data
        ...
        if past_key_values is not None:
            input_ids = input_ids[:, -1].reshape(input_ids.shape[0], -1)
        ...
        lm_logits = transformer_outputs[0]  # for ascend
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if rms_norm is not None and x.is_npu:
