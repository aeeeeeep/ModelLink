...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.c_attn = nn.Linear(config.hidden_size, 3 * self.num_heads * self.head_dim)  # 3 * 20 * 每头维度，作用是生成三个线性矩阵（注意，这里是单卡的）
        
        # self.c_proj = nn.Linear(
        #     config.hidden_size, self.num_heads * self.head_dim, bias=not config.no_bias
        # )
        self.c_proj = nn.Linear(self.num_heads * self.head_dim, config.hidden_size, bias=not config.no_bias)
        ################################################
    ...
    def forward(
        ...
        query, key, value = mixed_x_layer.split(self.num_heads * self.head_dim, dim=2)
        ###########################################################
...
class QWenBlock(nn.Module):
    ...
    def forward(
        ...
        ######################################################
        # For model cut  --- reduce
        if self.rank_size > 1:
            torch.distributed.all_reduce(mlp_output, op=torch.distributed.ReduceOp.SUM)
        ######################################################
