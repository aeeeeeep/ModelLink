...
if QUANT_WEIGHT_PATH is None or not os.path.exists(QUANT_WEIGHT_PATH):
    raise RuntimeError("QUANT_WEIGHT_PATH does not exist!")
FLOAT_LAYERS = [0, 1, 2, 3, 39]
MAX_SEQ_LENGTH = 1024 + 256 # 自定义最大输入输出长度，默认值2048
...
class GPTBigCodeBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if not RUN_QUANT_MODEL or layer_idx in FLOAT_LAYERS:
...
class GPTBigCodeModel(GPTBigCodePreTrainedModel):
    def __init__(self, config):
        ...
        self.max_position_embeddings = math.ceil(MAX_SEQ_LENGTH  / 16) * 16
        ...
        part_quant_weight_path = os.path.join(quant_weight_path, str(rank))
        self.quant_weight_dict = np.load(os.path.join(part_quant_weight_path, "quant_weight.npy"), allow_pickle=True).item()
        self.deq_scale_dict = np.load(os.path.join(part_quant_weight_path, "deq_scale.npy"), allow_pickle=True).item()
        self.quant_bias_dict = np.load(os.path.join(part_quant_weight_path, "bias.npy"), allow_pickle=True).item()
        # load quant input scale and offset
        self.input_scale_dict = np.load(os.path.join(part_quant_weight_path, "input_scale.npy"), allow_pickle=True).item()
        ...
        self.mlp_out_input_scale = []
        self.mlp_out_input_offset = []
        ...
        for layer_id in range(self.layer_num):
            if layer_id in self.float_layers:
                ...
                self.mlp_out_input_scale.append(float(0))
                self.mlp_out_input_offset.append(float(0))
            else:
                qkv_name = "transformer.h.{}.attn.c_attn".format(layer_id)
                c_name = "transformer.h.{}.attn.c_proj".format(layer_id)
                up_name = "transformer.h.{}.mlp.c_fc".format(layer_id)
                down_name = "transformer.h.{}.mlp.c_proj".format(layer_id)
                self.qkv_input_scale.append(float(1/self.input_scale_dict[qkv_name]))
                self.qkv_input_offset.append(int(self.input_offset_dict[qkv_name]))
                self.dense_input_scale.append(float(1/self.input_scale_dict[c_name]))
                self.dense_input_offset.append(int(self.input_offset_dict[c_name]))
                self.self_ln_input_scale.append(float(1/self.input_scale_dict[up_name]))
                self.self_ln_input_offset.append(int(self.input_offset_dict[up_name]))
                self.mlp_out_input_scale.append(float(1/self.input_scale_dict[down_name]))
                self.mlp_out_input_offset.append(int(self.input_offset_dict[down_name]))

        self.acl_param_en = json.dumps({
            "headNum": self.n_head, 
            ...
            "mlpOutInputScale": self.mlp_out_input_scale, "mlpOutInputOffset": self.mlp_out_input_offset,
            "floatLayers": self.float_layers,
            "isEncoder": True
            })
        
        self.acl_param_de = json.dumps({
            "headNum": self.n_head, 
            "layerNormEps": self.layer_norm,
            "dk": self.head_dim, 
            "layerNum": self.layer_num,
    ...
    def init_acl_weight(self):
        ...
        for i in range(self.layer_num):
            ...
            if i in self.float_layers:
                ...
                qkv_name = "transformer.h.{}.attn.c_attn".format(i)
                c_name = "transformer.h.{}.attn.c_proj".format(i)
                up_name = "transformer.h.{}.mlp.c_fc".format(i)
                down_name = "transformer.h.{}.mlp.c_proj".format(i)

                weights.append(weights_layer[str_keys + "ln_1.weight"])
                weights.append(weights_layer[str_keys + "ln_1.bias"])
                # qkv量化
                weights.append(self.transdata_operation.execute([self.quant_weight_dict[qkv_name].to(torch.int8).npu()])[0])
                weights.append(self.quant_bias_dict[qkv_name].to(torch.int32).npu())
                weights.append(self.deq_scale_dict[qkv_name].to(torch.int64).npu())
                weights.append(self.transdata_operation.execute([self.quant_weight_dict[c_name].to(torch.int8).npu()])[0])
                weights.append(self.quant_bias_dict[c_name].to(torch.int32).npu())
                weights.append(self.deq_scale_dict[c_name].to(torch.int64).npu())

                weights.append(weights_layer[str_keys + "ln_2.weight"])
                weights.append(weights_layer[str_keys + "ln_2.bias"])
                # mlp量化
                weights.append(self.transdata_operation.execute([self.quant_weight_dict[up_name].to(torch.int8).npu()])[0])
                weights.append(self.quant_bias_dict[up_name].to(torch.int32).npu())
                weights.append(self.deq_scale_dict[up_name].to(torch.int64).npu())
                weights.append(self.transdata_operation.execute([self.quant_weight_dict[down_name].to(torch.int8).npu()])[0])
                weights.append(self.quant_bias_dict[down_name].to(torch.int32).npu())
                weights.append(self.deq_scale_dict[down_name].to(torch.int64).npu())
    ...
    def init_mask_to_atb(self, mask, max_seq_out):
        ...
        acl_shape_en = attention_mask_acl_en.shape
        attention_mask_acl_en = attention_mask_acl_en.view(acl_shape_en[0], acl_shape_en[1], acl_shape_en[2]//16, 16) \
            .transpose(1, 2).contiguous()

        attention_mask_acl_de = attention_mask_acl_de.unsqueeze(-2).repeat(1, 16, 1)
        acl_shape = attention_mask_acl_de.shape
        attention_mask_acl_de = attention_mask_acl_de.view(acl_shape[0], acl_shape[1], acl_shape[2] // 16, 16) \
            .transpose(1,2).contiguous()

        acl_shape = attention_mask_acl_en.shape
        self.attention_mask_max_en[:acl_shape[0], :acl_shape[1], :acl_shape[2], :acl_shape[3]] += attention_mask_acl_en
        acl_shape = attention_mask_acl_de.shape
        self.attention_mask_max_de[:acl_shape[0], :acl_shape[1], :acl_shape[2], :acl_shape[3]] += attention_mask_acl_de
    ...
    def forward(
        ...
        if past_key_values is None:
            past_length = 0
            past_key_values = tuple([None] * self.layer_num)
            self.isEncoder = True
        else:
        ...
        if batch_size != self.batch:
            print(">>>>>>>>>>init kv_cache and mask input<<<<<<<<<<<<<<<<")
            ...
            self.k_cache_input = torch.zeros(self.layer_num,
                                             ...
                                             16,
                                             dtype=torch.half, device="npu").contiguous()
            self.v_cache_input = torch.zeros(self.layer_num,
                                             batch_size,
                                             self.head_dim // 16,
                                             self.max_position_embeddings,
                                             16,
                                             dtype=torch.half, device="npu").contiguous() #[40,bsz,8,8192,16]
            ...
            self.attention_mask_max_en = torch.zeros(
                (self.batch, math.ceil(self.max_position_embeddings / 16), self.max_position_embeddings, 16),
                device='npu',
                dtype=torch.half
            ).contiguous()
