...
class GPTBigCodeModel(GPTBigCodePreTrainedModel):
    def __init__(self, config):
        ...
        self.layerNormEps = config.layer_norm_epsilon
        self.layerNum = config.num_hidden_layers
        ...
        self.acl_param = json.dumps({"headNum": self.n_head, 
                                     "layerNormEps": self.layerNormEps,
                                     "dk": self.head_dim, 
                                     "layerNum": self.layerNum,
                                     "rank": rank, 
                                     "rankSize": rankSize})
        
        self.acl_fa_model.set_param(self.acl_param)
        print(">>> set ATB param")
        ...
        self.acl_encoder_operation_inputs = [None] * (7 + self.layerNum)
        self.acl_decoder_operation_inputs = [None] * (7 + self.layerNum)
        for i in range(self.layerNum):
            self.acl_encoder_operation_inputs[7 + i] = torch.tensor([i], dtype=torch.int32).npu()
    ...
    def init_acl_weight(self):
        ...
        self.acl_fa_model.set_weight(self.acl_weights)
    ...
    def prepare_inputs_for_acl(self, input_ids, position_ids, seq_length, batch_size, past_key_values=None):
        ...
        if not past_key_values or past_key_values[0] is None:
            ...
            if not self.is_910b:
                ...
                self.attention_mask_encoder = self.attention_mask_encoder.permute(0,2,1,3)
                self.attention_mask_encoder.data = torch_npu.npu_format_cast(self.attention_mask_encoder.data,29)

            self.acl_encoder_operation_inputs[0] = input_ids
            self.acl_encoder_operation_inputs[1] = position_ids
            self.acl_encoder_operation_inputs[2] = self.attention_mask_encoder
            ...
            if self.is_910b:
                ...
                self.attention_mask_decoder = self.attention_mask_decoder.view(batch_size, self.max_position_embeddings, self.max_position_embeddings // 16, 16)
                self.attention_mask_decoder = self.attention_mask_decoder.permute(0,2,1,3)
                self.attention_mask_decoder.data = torch_npu.npu_format_cast(self.attention_mask_decoder.data,29)

            self.acl_decoder_operation_inputs[0] = input_ids
            self.acl_decoder_operation_inputs[1] = position_ids
            self.acl_decoder_operation_inputs[2] = self.attention_mask_decoder
...
# ATB===================================================================================================================================
        if batch_size != self.batch:
            ...
            if self.is_910b:
                ...
                self.v_cache_input = torch.zeros(self.layerNum,
                                                batch_size,
                                                self.max_position_embeddings,
                ...
                self.v_cache_input = torch.zeros(self.layerNum,
                                                ...
                                                self.max_position_embeddings,
                                                16,
                                                dtype=torch.half, device="npu").contiguous() #[40,bsz,8192,128]
        ...
        hidden_states = self.execute_acl_operator(self.acl_fa_model, input_ids, position_ids, batch_size, query_length, past_key_values)
...
class GPTBigCodeForCausalLM(GPTBigCodePreTrainedModel):
    ...
    def forward(
        ...
        # print(">>>>>>>>>>>>>>>> save first tensor success <<<<<<<<<<<<<<<<<<")
        # sys.exit()
