...
# Quant weight path
QUANT_WEIGHT_PATH = "./internlm20b_quant_weight"
# Anti-outlier quant weight path
ANTI_QUANT_WEIGHT_PATH = "./internlm20b_quant_weight/anti_weight"

# Sparse weight path
COMPRESS_WEIGHT_PATH = "./internlm20b_sparsequant_weight"
...
ALL_LAYERNORM_LAYERS.append(InternlmRMSNorm)
...
class InternlmLinearScalingRotaryEmbedding(InternlmRotaryEmbedding):
    """InternlmRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class InternlmDynamicNTKScalingRotaryEmbedding(InternlmRotaryEmbedding):
    """InternlmRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""
...
class InternlmAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.rope_scaling is None:
            local_rotary_emb = InternlmRotaryEmbedding(
                self.head_dim,
            ...
            if scaling_type == "linear":
                local_rotary_emb = InternlmLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                local_rotary_emb = InternlmDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
class InternlmModel(InternlmPreTrainedModel):
    ...
    def __init__(self, config: InternLMConfig):
        ...
        self.layers = nn.ModuleList([InternlmDecoderLayer(config, i) for i in range(config.num_hidden_layers)])
        self.norm = InternlmRMSNorm(config.hidden_size, eps=config.rms_norm_eps)

        # initialize dtype
        self.isBF16 = (self.dtype == torch.bfloat16)
        print(f"InternlmModel, dtype: {self.dtype}")
    ...
    def init_ascend_operations(self):
        ...
        self.acl_encoder_operation = torch.classes.ModelTorch.ModelTorch("internlm_20b_FlashAttentionRopeModel")
        self.acl_decoder_operation = torch.classes.ModelTorch.ModelTorch("internlm_20b_FlashAttentionRopeModel")
