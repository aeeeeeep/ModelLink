...
def load_anti_outlier_state_dict(anti_weight_path):
    ...
    for weight_file in weight_files:
        ...
        anti_outlier_dict.update(weight_tensor_dict)
    return anti_outlier_dict
...
def process_deq_scale(deq_scale_tensor):
    """
    deq转成int64
    """
...
class InternLMModel(InternLMPreTrainedModel):
    ...
    def init_ascend_operations(self, config: InternLMConfig):
        ...
        self.acl_param = json.dumps({
            ...
            "backend": "lccl",
            "qProjInputScale": q_proj_input_scale,
            "qProjInputOffset": q_proj_input_offset,
            "kProjInputScale": k_proj_input_scale,
            "kProjInputOffset": k_proj_input_offset,
            "vProjInputScale": v_proj_input_scale,
            "vProjInputOffset": v_proj_input_offset,
            "oProjInputScale": o_proj_input_scale,
            "oProjInputOffset": o_proj_input_offset,
            "gateProjInputScale": gate_proj_input_scale,
            "gateProjInputOffset": gate_proj_input_offset,
            "downProjInputScale": down_proj_input_scale,
            "downProjInputOffset": down_proj_input_offset
        })
        self.max_position_embeddings = config.max_position_embeddings
    ...
    def init_ascend_weight(self):
        ...
        anti_weight_no_bias_list = [anti_o_proj_name, anti_down_proj_name]
        ...
        for layer_index in range(self.num_layers):
            if layer_index in self.float_layers:
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[q_proj_name]).npu())
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[k_proj_name]).npu())
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[v_proj_name]).npu())
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[o_proj_name]).npu())
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[up_proj_name]).npu())
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[gate_proj_name]).npu())
                ...
                self.weights.append(process_deq_scale(deq_scale_dict[down_proj_name]).npu())
