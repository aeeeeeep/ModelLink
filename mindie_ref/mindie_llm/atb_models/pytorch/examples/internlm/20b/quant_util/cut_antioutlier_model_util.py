...
import argparse
import torch
import antioutlier_model_util

from transformers import AutoConfig, AutoTokenizer, AutoModelForCausalLM
from modeling_internlm_quant_antioutlier_gen import InternLMForCausalLM
from antioutlier_model_util import replace_module
...
if __name__ == "__main__":
    ...
    model = replace_module(model, model.config)
    ...
    create_model = InternLMForCausalLM(create_config)
    for i in range(args.world_size):
        create_model.load_state_dict(state_dict_list[i]) # load the weights to the model
        create_model.save_pretrained(os.path.join(args.output_path, 'part_model', str(i))) # save model
