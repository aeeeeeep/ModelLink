...
class TelechatModel(TelechatPreTrainedModel):
    def __init__(self, config: TelechatConfig):
        ...
        # print([i.device for i in self.state_dict().values()])
   ...
   #         layer.self_attention.kv_cache = None
    ...
    def forward(
        ...
        if self.weightFlag is False:
            ...
            weight_layer = self.state_dict()
            self.weights.append(weight_layer["word_embeddings.weight"])
            for layer_idx in range(self.num_hidden_layers):
                self.weights.append(weight_layer[f"h.{layer_idx}.self_attention.query.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.self_attention.key_value.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.self_attention.dense.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.self_attention.dense.bias"])
                self.weights.append(weight_layer[f"h.{layer_idx}.mlp.gate_proj.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.mlp.up_proj.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.mlp.down_proj.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.mlp.down_proj.bias"])
                self.weights.append(weight_layer[f"h.{layer_idx}.input_layernorm.weight"])
                self.weights.append(weight_layer[f"h.{layer_idx}.post_attention_layernorm.weight"])
            self.weights.append(weight_layer[f"ln_f.weight"])
            # print(list(weight_layer.keys()))
