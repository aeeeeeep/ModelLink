...
W8A16Operation::W8A16Operation(const std::string &name) : AclNnOperation(name) {}

W8A16Operation::~W8A16Operation() {}

atb::Status W8A16Operation::InferShape(const atb::SVector<atb::TensorDesc> &inTensorDescs,
...
uint32_t W8A16Operation::GetInputNum() const { return NUM4; }

uint32_t W8A16Operation::GetOutputNum() const { return NUM1; }

int W8A16Operation::CreateAclTensors(const atb::VariantPack &variantPack, AclNnTask &task)
{
    ATB_LOG(INFO) << opName_ << " CreateAclTensor start";
    task.aclInTensors.resize(NUM4);
...
int W8A16Operation::CallAclGetWorkspace(const atb::VariantPack &variantPack, AclNnTask &task,
                                        uint64_t &workspaceSize)
{
    ...
    int ret = aclnnWeightQuantBatchMatmulV2GetWorkspaceSize(
        ...
        task.aclInTensors.at(3).tensor, nullptr, nullptr, nullptr, 0,
...
int W8A16Operation::CallAclExecute(uint8_t *workspace, uint64_t workspaceSize,
...
AclNnTensor W8A16Operation::CreateTensor(atb::Tensor atbTensor)
{
