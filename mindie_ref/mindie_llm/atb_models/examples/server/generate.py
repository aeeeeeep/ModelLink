...
def next_token_topktopp(temperature, top_k, top_p, min_tokens_to_keep, logits: torch.Tensor):
    topktopp_OP_NAME = torch.classes.OperationTorch.OperationTorch("Layerstopktopp")
    topktopp_PARAM = json.dumps({"axes": 1, "headNum": 0, "topk": top_k, "vocsize": logits.size()[1], "row":logits.size()[0],"randseed":random.randint(1,100),"min_tokens_to_keep": min_tokens_to_keep})
    topktopp_OP_NAME.set_param(topktopp_PARAM)
    next_logits, next_token=topktopp_OP_NAME.execute([logits,torch.HalfTensor([top_p]).npu(),torch.HalfTensor([temperature]).npu()])
    return next_token

def generate_token(model, tokenizer, cache_manager, batch: Batch, max_out_length, rank, ignore_eos, temperature, top_k, top_p, min_tokens_to_keep):
    ...
    next_token = next_token_topktopp(temperature, top_k, top_p, min_tokens_to_keep, logits)
...
def generate_req(req_list, model, tokenizer,
                 ...
                 rank, ignore_eos, temperature, top_k, top_p, min_tokens_to_keep):
            ...
            if prefill_batch_size > 0:
                ...
                if ENV.benchmark_enable:
                    ...
                    req_finished = generate_token(model, tokenizer, cache_manager, batch, max_out_length, rank,
                                                  ignore_eos, temperature, top_k, top_p, min_tokens_to_keep)
                    torch.npu.synchronize()
