...
class NpuHbmInfo:
    ...
    def set_visible_devices(cls, world_size):
        ...
        if ENV.visible_devices is None:
            ...
            devices = list(map(int, ENV.visible_devices.split(',')))
    ...
    def get_hbm_capacity(cls, rank, world_size, need_nz):
        ...
        memory_info = execute_command(f"npu-smi info -i {npu_id} -t memory").split("\n")[1:]
        if not need_nz:
    ...
    def get_hbm_usage(cls, rank, world_size, need_nz):
        ...
        if not cls.visible_npu_ids:
            cls.set_visible_devices(world_size)
        npu_id = cls.visible_npu_ids[rank]
        usage_info = execute_command(f"npu-smi info -i {npu_id} -t usages").split("\n")[1:]
        if not need_nz:
...
def _get_pcie_info(devices: List[int], keyword="PCIeBusInfo"):
    ...
    for device in devices:
        ...
        pcie_info = execute_command(
            f"npu-smi info -t board -i {device_info.npu_id} -c {device_info.chip_id}").strip().split("\n")
        for _ in pcie_info:
...
def _get_numa_info(pcie_tbl, keyword="NUMAnode"):
    ...
    for device, pcie_no in pcie_tbl.items():
        numa_info = execute_command(f"lspci -s {pcie_no} -vvv").split("\n")
        for _ in numa_info:
            line = ''.join(_.split())
            if line.startswith(keyword):
...
def _get_cpu_info(numa_ids, keyword1="NUMAnode", keyword2="CPU(s)"):
    ...
    cpu_info = execute_command(f"lscpu").split("\n")
    for _ in cpu_info:
        line = ''.join(_.split())
