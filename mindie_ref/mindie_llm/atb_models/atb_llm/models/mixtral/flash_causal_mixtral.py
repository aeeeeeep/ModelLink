...
from .modeling_mixtral import MixtralConfig
...
class FlashMixtralForCausalLM(FlashForCausalLM):
    ...
    def init_ascend_operations(self, config: MixtralConfig):
        self.acl_encoder_operation = torch.classes.ModelTorch.ModelTorch("mixtralDense_DecoderModel")
        self.acl_decoder_operation = torch.classes.ModelTorch.ModelTorch("mixtralDense_DecoderModel")
    ...
    def init_ascend_weight(self):
        ...
        for i in range(self.num_layers):
            ...
            # add gate weights
            weights_t.append(self.weights.get_tensor(f"model.layers.{i}.block_sparse_moe.gate.weight").npu())

            # add common experts
            COMMON_EXPERTS_NUM = 8
            if self.tp:
                for j in range(COMMON_EXPERTS_NUM):
                    experts_gate_up = load_column_multi(
                        self.config,
                        prefixes=[f"model.layers.{i}.block_sparse_moe.experts.{j}.w1",
                                f"model.layers.{i}.block_sparse_moe.experts.{j}.w3"],
                        weights=self.weights,
                        head_size=1
                    ).npu()
                    ...
                    experts_down_proj = TensorParallelRowLinear.load(
                        self.config,
                        prefix=f"model.layers.{i}.block_sparse_moe.experts.{j}.w2",
                        weights=self.weights,
                        bias=False,
                    ).npu()
                ...
                for j in range(COMMON_EXPERTS_NUM):
                    if j < expert_per_rank:
                        ...
                        weights_t.append(torch.cat([self.weights.get_tensor(f"model.layers.{i}.block_sparse_moe.experts.{exprt_id}.w1.weight"),
                                                    self.weights.get_tensor(f"model.layers.{i}.block_sparse_moe.experts.{exprt_id}.w3.weight")]).npu())

                        weights_t.append(self.weights.get_tensor(f"model.layers.{i}.block_sparse_moe.experts.{exprt_id}.w2.weight").npu())
                    else:   
                        weights_t.append(self.placeholder)
                        weights_t.append(self.placeholder)
