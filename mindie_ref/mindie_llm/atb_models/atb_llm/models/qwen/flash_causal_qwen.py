        ...
        if config.use_refactor:
            ...
            if self.num_key_value_heads != self.num_attention_heads:
                ...
                if self.quantize == "w8a8":
                    # param在init_ascend_weight()中加载，因为其中部分字段需要从量化权重中获取，比如linearQuantType
    ...
    def init_ascend_weight(self):
        if self.use_refactor:
            ...
            if self.quantize is None:
                logger.info(">>>> fp16")
                weights = [self.transformer.state_dict()["wte.weight"]]
                attn_layer_weight_names = [
                    'attn.c_attn.linear.weight',
                    'attn.c_attn.linear.bias',
                    'attn.c_proj.linear.weight'
                ]
                mlp_layer_names = [
                    'mlp.w2.linear',  # gate_proj
                    'mlp.w1.linear',  # up_proj
                    'mlp.c_proj.linear'  # down_proj
                ]
                for i in range(self.num_layers):
                    weights_t = []
                    ...
                    if self.num_attention_heads != self.num_key_value_heads:
                        ...
                        for layer_weight_name in attn_layer_weight_names:
                            weights_t.append(weights_layer[f'{layer_weight_name}'])
                        ...
                        weights_t.append(weights_layer["ln_2.weight"])
                        ...
                        weights_t.append(weights_layer["ln_1.weight"])
                        weights_t.append(self.weight_format_cast(weights_layer["attn.c_attn.linear.weight"]))
                        weights_t.append(self.weight_format_cast(weights_layer["attn.c_attn.linear.bias"]))
                        weights_t.append(self.weight_format_cast(weights_layer["attn.c_proj.linear.weight"]))
                        weights_t.append(weights_layer["ln_2.weight"])
                        weights_t.append(self.weight_format_cast(weights_layer["mlp.w2_w1.linear.weight"]))
                        weights_t.append(self.weight_format_cast(weights_layer["mlp.c_proj.linear.weight"]))

                    if self.soc_info.need_nz:
                        del self.transformer.h[i].attn
                        del self.transformer.h[i].ln_2
                        del self.transformer.h[i].mlp
                    weights.extend(weights_t)

                weights.append(self.transformer.state_dict()["ln_f.weight"])
