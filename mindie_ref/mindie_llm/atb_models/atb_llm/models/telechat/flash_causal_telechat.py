...
def _expand_mask(
    mask: torch.Tensor, dtype: torch.dtype, tgt_length: int
) -> torch.BoolTensor:
    """
    ...
    expanded_mask = (
        mask[:, None, None, :].expand(batch_size, 1, tgt_length, src_length).to(dtype)
    )
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, dim, base=10000, precision=torch.half):
        ...
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float().double() / dim))
...
class TelechatAttention(nn.Module):
    def __init__(self, config: TelechatConfig, layer_idx):
        ...
        self.query = nn.Linear(
            self.hidden_size, self.hidden_size // self.world_size, bias=False
        )
        self.key_value = nn.Linear(
            self.hidden_size, kv_projection_size * 2 // self.world_size, bias=False
        )
    ...
    def __init__(self, config: TelechatConfig, layer_idx):
        ...
        if not RUN_QUANT_MODEL:
            self.gate_proj = nn.Linear(
                hidden_size, config.ffn_hidden_size // self.world_size, bias=False
            )
        ...
        if not RUN_QUANT_MODEL or layer_idx in FLOAT_DOWN_LAYERS:
            self.down_proj = nn.Linear(
                config.ffn_hidden_size // self.world_size, hidden_size, bias=True
            )

    def swiglu(self, x):
        x = torch.chunk(
            x, 2, dim=-1
        )  ###split x into 2 tensors along the last dimension
...
class TelechatBlock(nn.Module):
    def __init__(self, config: TelechatConfig, layer_idx):
        ...
        self.input_layernorm = TelechatRMSNorm(
        ...
        self.post_attention_layernorm = TelechatRMSNorm(
            hidden_size, eps=config.layer_norm_epsilon
        )
...
def get_bias_correction_dict(
    fp_bias_dict, quant_weight_dict, input_offset_dict, deq_scale_dict
):
    bias_dict = {}
    for k in fp_bias_dict.keys():
        bias_dict[k] = bias_correction_new(
            fp_bias_dict[k],
            quant_weight_dict[k],
            input_offset_dict[k],
            deq_scale_dict[k],
...
class TelechatModel(TelechatPreTrainedModel):
    def __init__(self, config: TelechatConfig):
        ...
        self.h = nn.ModuleList(
            [TelechatBlock(config, _) for _ in range(config.num_hidden_layers)]
        )
        # Final Layer Norm
        ...
        if RUN_QUANT_MODEL:
            ...
            if self.world_size > 1:
                quant_param_path = os.path.join(
                    QUANT_WEIGHT_PATH, "part_model", str(self.rank)
                )
            else:
                quant_param_path = QUANT_WEIGHT_PATH
            self.input_scale_dict = np.load(
                os.path.join(quant_param_path, "input_scale.npy"), allow_pickle=True
            ).item()
            self.input_offset_dict = np.load(
                os.path.join(quant_param_path, "input_offset.npy"), allow_pickle=True
            ).item()
            self.quant_weight_dict = np.load(
                os.path.join(quant_param_path, "quant_weight.npy"), allow_pickle=True
            ).item()
            self.weight_scale_dict = np.load(
                os.path.join(quant_param_path, "weight_scale.npy"), allow_pickle=True
            ).item()
            self.deq_scale_dict = np.load(
                os.path.join(quant_param_path, "deq_scale.npy"), allow_pickle=True
            ).item()
            self.fp_bias_dict = np.load(
                os.path.join(quant_param_path, "fp_bias.npy"), allow_pickle=True
            ).item()
            if self.world_size == 1:
                self.fp_bias_dict = get_bias_correction_dict(
                    self.fp_bias_dict,
                    self.quant_weight_dict,
                    self.input_offset_dict,
                    self.deq_scale_dict,
                )
        ...
        for layer_idx in range(self.num_hidden_layers):
            ...
            key_value_name = "transformer.h.{}.self_attention.key_value".format(
                layer_idx
            )
            ...
            if RUN_QUANT_MODEL:
                dense_input_scale[layer_idx] = float(
                    1 / self.input_scale_dict[dense_name]
                )
                dense_input_offset[layer_idx] = int(self.input_offset_dict[dense_name])
                gate_up_input_scale[layer_idx] = float(
                    1 / self.input_scale_dict[gate_proj_name]
                )
                gate_up_input_offset[layer_idx] = int(
                    self.input_offset_dict[gate_proj_name]
                )

            if layer_idx not in self.float_query_layers:
                qkv_input_scale[layer_idx] = float(
                    1 / self.input_scale_dict[query_name]
                )
            ...
            if layer_idx not in self.float_down_layers:
                down_proj_input_scale[layer_idx] = float(
                    1 / self.input_scale_dict[down_proj_name]
                )
                down_proj_input_offset[layer_idx] = int(
                    self.input_offset_dict[down_proj_name]
                )
        ...
        for i in range(self.num_hidden_layers):
            self.acl_inputs[i + num_intensors] = torch.tensor(
                [i], dtype=torch.int32
            ).npu()
        ...
        self.maskAttenincreCache = torch.full(
            (self.max_seq_len, self.max_seq_len),
            torch.finfo(torch.float16).min,
            device="npu",
            dtype=torch.half,
        )
        self.maskAttenincreZero = torch.full(
            (self.max_seq_len, self.max_seq_len), 0, device="npu", dtype=torch.half
        )
        ...
        for layer_idx in range(self.num_hidden_layers):
            ...
            if layer_idx in self.float_query_layers:
                self.model_weights[layer_idx].append(
                    weights_float["self_attention.query.weight"].npu()
                )
            else:
                self.model_weights[layer_idx].append(
                    transdata_operation.execute(
                        [self.quant_weight_dict[query_name].to(torch.int8).npu()]
                    )[0]
                )
            if layer_idx in self.float_kv_layers:
                self.model_weights[layer_idx].append(
                    weights_float["self_attention.key_value.weight"].npu()
                )
            else:
                self.model_weights[layer_idx].append(
                    transdata_operation.execute(
                        [self.quant_weight_dict[key_value_name].to(torch.int8).npu()]
                    )[0]
                )

            self.model_weights[layer_idx].append(
                transdata_operation.execute(
                    [self.quant_weight_dict[dense_name].to(torch.int8).npu()]
                )[0]
            )
            self.model_weights[layer_idx].append(
                transdata_operation.execute(
                    [self.quant_weight_dict[gate_proj_name].to(torch.int8).npu()]
                )[0]
            )
            self.model_weights[layer_idx].append(
                transdata_operation.execute(
                    [self.quant_weight_dict[up_proj_name].to(torch.int8).npu()]
                )[0]
            )

            if layer_idx in self.float_down_layers:
                self.model_weights[layer_idx].append(
                    weights_float["mlp.down_proj.weight"].npu()
                )
            else:
                self.model_weights[layer_idx].append(
                    transdata_operation.execute(
                        [self.quant_weight_dict[down_proj_name].to(torch.int8).npu()]
                    )[0]
                )
    ...
    def _prepare_attn_mask(
        ...
        expanded_attn_mask = _expand_mask(
            attention_mask, torch.float16, tgt_length=src_length
        )
    ...
    def forward(
        ...
        **deprecated_arguments,
        ...
        if self.encoder_flag:
            self.maskAttenfull = torch.full(
                (self.batch_num, self.max_seq_len, self.max_seq_len),
                0,
                device="npu",
                dtype=torch.half,
            )
        ...
        if attention_mask is None:
            attention_mask = torch.ones(
                (batch_size, seq_length_with_past), device=input_ids.device
            )
        else:
        ...
        if self.weightFlag is False:
            ...
            for layer_idx in range(self.num_hidden_layers):
                ...
                self.weights.append(
                    weights_float["post_attention_layernorm.weight"].npu()
                )
                self.weights.append(
                    weights_float["post_attention_layernorm.bias"].npu()
                )
        ...
        if self.encoder_flag:
            if self.batch_num == 1:
                ...
                decoder_rightmask = self.maskAttenincreZero[
                    :, : self.max_seq_len - seq_length + count
                ]
                self.maskAttenincre = torch.concat(
                    [decoder_leftmask, decoder_rightmask], dim=-1
                ).unsqueeze(0)
            else:
                decoder_masks = []
                for i in range(self.batch_num):
                    ...
                    decoder_mask = torch.concat(
                        [decoder_leftmask, decoder_rightmask], dim=-1
                    ).unsqueeze(0)
                    decoder_masks.append(decoder_mask)
                self.maskAttenincre = torch.concat(decoder_masks, dim=0)
            self.maskAttenfull = torch_npu.npu_format_cast(
                self.maskAttenfull.view(
                    self.batch_num, self.max_seq_len, self.max_seq_len // 16, 16
                )
                .transpose(1, 2)
                .contiguous(),
                29,
            )
            self.maskAttenincre = torch_npu.npu_format_cast(
                self.maskAttenincre.view(
                    self.batch_num, self.max_seq_len, self.max_seq_len // 16, 16
                )
                .transpose(1, 2)
                .contiguous(),
            ...
            seqlen = torch.tensor(
                [self.seq_len] * batch_size, dtype=torch.int32, device=input_ids.device
            )
            ...
            self.seq_len += 1
            seqlen = torch.tensor(
                [1] * batch_size, dtype=torch.int32, device=input_ids.device
            )
            token_offset = torch.tensor(
                [self.seq_len] * batch_size, dtype=torch.int32, device=input_ids.device
            )
            ...
            self.acl_inputs[8] = seqlen
        param = json.dumps(
            {"tokenOffset": token_offset.tolist(), "seqLen": seqlen.tolist()}
        )
...
class TelechatForCausalLM(TelechatPreTrainedModel):
    ...
    def chat(
        ...
        tokenizer,
        question: str = "",
        history: Union[List[Dict], History] = None,
        stream: bool = False,
        ...
        **kwargs,
        ...
        inputs = self.build_inputs_for_chat(
            tokenizer, question, history, generation_config, user_id, bot_id
        )
    ...
    def build_inputs_for_chat(
        ...
        build_max_length = (
            max(0, model_max_length - generation_config.max_new_tokens)
            if generation_config.max_new_tokens
            else max(0, generation_config.max_length)
        )
        if build_max_length < 3:
            logger.warning(
                "the model can not meet the  requirements of input length,Please check config"
            )
            raise ValueError("")

        # trunc left
        input_tokens = (
            [usr_id] + q_token["input_ids"][-build_max_length + 1 :] + [bot_id]
        )
        ...
        while len(qa_history) != 0:
            ...
            if message["role"] == "user":
                ...
                tokens = (
                    [bot_id] + message["input_ids"] + [generation_config.eos_token_id]
                )
            else:
                tokens = []
...
class TelechatForQuestionAnswering(TelechatPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        TELECHAT_INPUTS_DOCSTRING.format("batch_size, sequence_length")
    )
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
