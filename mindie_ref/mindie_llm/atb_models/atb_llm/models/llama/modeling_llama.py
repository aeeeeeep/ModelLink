...
from atb_llm.utils.data_processing import WeightATB
...
class FlashLlamaForCausalLM(FlashForCausalLM):
    ...
    def init_ascend_weight(self):
        if self.use_refactor:
            weight_atb = WeightATB(self.soc_info, self.tp_rank)
            weight_atb.register_embedding(self.model.state_dict(), 'embed_tokens')
            for i in range(self.num_layers):
                layer = self.model.layers[i]
                layer_dict = layer.state_dict()
                if layer.self_attn.pack_type in [PackType.ALL_FP, PackType.ALL_INT, PackType.ALL_ANTI]:
                    weight_atb.register_layer_linear_pack(layer_dict, 'input_layernorm',
                                                          'self_attn.query_key_value',
                                                          layer.self_attn.pack_type)
                else:
                    if layer.self_attn.pack_type == PackType.MIX_FP_INT:
                        weight_atb.register_layer_norm(layer_dict, 'input_layernorm')
                    else:
                        weight_atb.register_layer_norm_anti(layer_dict, 'input_layernorm_anti')
                    weight_atb.register_layer_linear(layer_dict, 'self_attn.q_proj')
                    weight_atb.register_layer_linear(layer_dict, 'self_attn.k_proj')
                    weight_atb.register_layer_linear(layer_dict, 'self_attn.v_proj')
                weight_atb.register_layer_linear(layer_dict, 'self_attn.o_proj')

                if layer.mlp.pack_type in [PackType.ALL_FP, PackType.ALL_INT, PackType.ALL_ANTI]:
                    weight_atb.register_layer_linear_pack(layer_dict, 'post_attention_layernorm',
                                                          'mlp.gate_up_proj',
                                                          layer.mlp.pack_type)
                else:
                    if layer.mlp.pack_type == PackType.MIX_FP_INT:
                        weight_atb.register_layer_norm(layer_dict, 'post_attention_layernorm')
                    else:
                        weight_atb.register_layer_norm_anti(layer_dict, 'post_attention_layernorm_anti')
                    weight_atb.register_layer_linear(layer_dict, 'mlp.gate_proj')
                    weight_atb.register_layer_linear(layer_dict, 'mlp.up_proj')
                weight_atb.register_layer_linear(layer_dict, 'mlp.down_proj')
                if self.soc_info.need_nz:
                    del layer.self_attn
                    del layer.post_attention_layernorm
                    del layer.mlp
            weight_atb.register_model_norm(self.model.state_dict(), 'norm')
            weight_atb.register_model_lmhead(self.state_dict(), 'lm_head')
            self.ascend_weight = weight_atb.weights
            exit()
            self.acl_encoder_operation.set_weight(weight_atb.weights)
            self.acl_decoder_operation.set_weight(weight_atb.weights)
        else:
