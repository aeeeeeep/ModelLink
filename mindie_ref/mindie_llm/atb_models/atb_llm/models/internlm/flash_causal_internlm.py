...
class InternLMAttention(nn.Module):
    ...
    def __init__(self, prefix, config, weights):
        ...
        self.q_proj = TensorParallelRowLinear.load(
            config,
            prefix=f"{prefix}.q_proj",
            weights=weights,
            bias=config.bias,
        )
        self.k_proj = TensorParallelRowLinear.load(
            config,
            prefix=f"{prefix}.k_proj",
            weights=weights,
            bias=config.bias,
        )
        self.v_proj = TensorParallelRowLinear.load(
            config,
...
class InternLMDecoderLayer(nn.Module):
    def __init__(self, layer_id, config, weights):
        ...
        self.input_layernorm = InternLMRMSNorm(prefix=f"{prefix}.input_layernorm", weights=weights, eps=config.rms_norm_eps)
        self.post_attention_layernorm = InternLMRMSNorm(prefix=f"{prefix}.post_attention_layernorm", weights=weights, eps=config.rms_norm_eps)
...
class InternLMModel(InternLMPreTrainedModel):
    ...
    def __init__(self, config, weights):
        ...
        self.layers = nn.ModuleList([InternLMDecoderLayer(layer_id=layer_id, config=config, weights=weights) for layer_id in range(config.num_hidden_layers)])
        ...
        self.ascend_attn_mask = AttentionMask.static(config.model_max_length)
    ...
    def prepare_inputs_for_ascend(self, 
        ...
        if self.soc_info.need_nz:
            ...
            attention_mask = self.ascend_attn_mask.get_attn_mask(pad_maxs, kv_cache[0][0].dtype, kv_cache[0][0].device)
