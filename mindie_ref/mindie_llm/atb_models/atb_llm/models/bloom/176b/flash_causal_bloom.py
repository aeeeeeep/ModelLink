...
def log_rank_0(msg):
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        with open("bloom_pa_log.txt", "a") as f:
            f.write(f"{msg}\n")
...
def _fill_with_neg_inf(t):
    """FP16-compatible function that fills a tensor with -inf.""" # float("-inf")
    return t.float().fill_(-10000).type_as(t)
...
class FlashBloomForCausalLM(BloomPreTrainedModel):
    ...
    def __init__(self, config, weights):
        ...
        # self.tp_rank, self.tp_world_size = get_distributed_info()
        ...
        param_dict = {
            ...
            "quantMode": config.quant_mode, "backend":"lccl"
            }
    ...
    def load_model(self, config):
        ...
        elif config.quant_mode == 1:
            return self.load_model_int8(config)
        elif config.quant_mode == 2:
    ...
    def get_alibi_mask(self, tensor, seq_length_with_past):
        if self.training:
            ...
            if self.first_run:
                ...
                if self.tp_rank == 0:
                    print("[debug] self.first_run future_mask =", self.future_mask)
