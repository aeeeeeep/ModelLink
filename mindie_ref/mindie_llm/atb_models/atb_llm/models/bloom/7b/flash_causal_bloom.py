...
from transformers import PreTrainedModel, BloomConfig
...
def register_layer_mlp_bloom(self, layer_dict, pack_type, quantize_type, mlp_module_names):
    if quantize_type == 'smooth_quant':
        ...
        if pack_type in [PackType.ALL_FP, PackType.ALL_W8A8, PackType.ALL_W8A8_ANTI,
            ...
            self.register_layer_linear(layer_dict, mlp_module_names.pack_name, quantize_type)  # difference
            self.register_layer_linear(layer_dict, mlp_module_names.pack_name, quantize_type)  # just for holder
...
class FlashBloomForCausalLM(FlashForCausalLM):
    ...
    def init_ascend_weight(self):
        if not self.use_refactor:  # fp16
            ...
            torch.npu.synchronize()
            weights = [
                self.model.state_dict()["word_embeddings.weight"],
                self.model.state_dict()["word_embeddings_layernorm.weight"],
                self.model.state_dict()["word_embeddings_layernorm.bias"],
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                weights_t.append(self.weight_format_cast(weights_layer["mlp.dense_h_to_4h.linear.weight"]))
                weights_t.append(self.weight_format_cast(weights_layer["mlp.dense_4h_to_h.linear.weight"]))
            ...
            weights.append(self.model.state_dict()["ln_f.bias"])
            ...
            self.acl_param_encoder = json.dumps({
                ...
                "supportSwiGLU": False,
                "layerNormEps": self.config.layer_norm_epsilon,
    ...
    def init_ascend_operations(self, config: BloomConfig):
        ...
        if self.use_refactor:  # fp16、wa8a
            self.acl_encoder_operation = torch.classes.ModelTorch.ModelTorch("bloom_7b_DecoderModel")
            self.acl_decoder_operation = torch.classes.ModelTorch.ModelTorch("bloom_7b_DecoderModel")
        else:  # fp16
            self.acl_param_encoder = json.dumps({
    ...
    def prepare_inputs_for_ascend(self, input_ids: torch.Tensor,
        ...
        self.acl_operation_inputs = [
            ...
            self.place_holder,  # position_ids
            self.place_holder,  # cos table
            self.place_holder,  # sin table
            attention_mask,  # attn mask
            block_tables.to(torch.int32),  # block tables
            slots.to(torch.int32),  # slots
            input_lengths.to(torch.int32),  # layer idx
            input_lengths.to(torch.int32),  # token offsets
            self.place_holder,  # place holder
            input_lengths.to(torch.int32),  # input_length
            lm_head_indices if self.is_prefill else self.lm_head_indices_fake,  # lm indices
    ...
    def get_weights(self):
        ...
        mlp_module_names = MlpModuleNames(
            ...
            pack_name='mlp.dense_h_to_4h',
            down_name='mlp.dense_4h_to_h'
        )
        weight_wrapper = WeightWrapper(self.soc_info, self.tp_rank, attn_module_names, mlp_module_names)
        weight_wrapper.register_embedding(self.model.state_dict(), 'word_embeddings')
        weight_wrapper.register_model_norm(self.model.state_dict(), 'word_embeddings_layernorm')
        for i in range(self.config.num_hidden_layers):
            layer = self.model.h[i]
            layer_dict = layer.state_dict()
            weight_wrapper.register_layer(layer_dict, layer.self_attention.pack_type, layer.mlp.pack_type, self.quantize)
