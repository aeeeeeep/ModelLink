...
class AquilaRMSNormBias(nn.Module):
    def __init__(self, prefix, weights, eps=1e-6):
        """
        AquilaRMSNorm is equivalent to T5LayerNorm
        """
...
class AquilaRMSNormWrapper(nn.Module):
    def __init__(self, prefix, weights, eps=1e-6):
        """
        AquilaRMSNorm is equivalent to T5LayerNorm
        """
        ...
        self.ori = AquilaRMSNorm(prefix, weights, eps)
        self.anti = AquilaRMSNormBias(f'{prefix}.module', weights, eps)
...
class FlashAquilaAttention(torch.nn.Module):
    def __init__(
            self,
            prefix: str,
            config,
...
class FlashAquilaLayer(nn.Module):
    def __init__(self, layer_id, config, weights):
        super().__init__()
        prefix = f"model.layers.{layer_id}"
        self.self_attn = FlashAquilaAttention(
        ...
        if self.self_attn.pack_type in [PackType.ALL_FP, PackType.ALL_W8A16]:
            ...
            self.input_layernorm = AquilaRMSNormBias(
                prefix=f"{prefix}.input_layernorm", weights=weights, eps=config.rms_norm_eps
            )
        else:
            self.input_layernorm = AquilaRMSNormWrapper(
                prefix=f"{prefix}.input_layernorm", weights=weights, eps=config.rms_norm_eps
            )
        if self.mlp.pack_type in [PackType.ALL_FP, PackType.ALL_W8A16]:
            ...
            self.post_attention_layernorm = AquilaRMSNormBias(
                prefix=f"{prefix}.post_attention_layernorm",
                weights=weights,
                eps=config.rms_norm_eps,
            )
        else:
            self.post_attention_layernorm = AquilaRMSNormWrapper(
...
class FlashAquilaModel(torch.nn.Module):
...
class AquilaAttention(torch.nn.Module):
    def __init__(
            self,
            prefix: str,
            config,
...
class AquilaModel(torch.nn.Module):
    def __init__(self, config, weights):
        super().__init__()
