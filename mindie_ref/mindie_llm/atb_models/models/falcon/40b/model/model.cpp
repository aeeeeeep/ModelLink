...
#include "models/falcon/40b/layer/layer.h"
...
enum OutTensorId : int {
    OUT_TENSOR_HIDDENSTATES = IN_TENSOR_MAX,
    OUT_TENSOR_MAX,
};
...
FusionModel::FusionModel(const std::string &param) : Model("FusionModel", param) { param_.FromString(param); }
...
int64_t FusionModel::BuildGraph()
{
    ...
    graph_.outTensors.resize(OUT_TENSOR_MAX - IN_TENSOR_MAX);
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        layerNode.inTensors.at(inTensorId++) = &graph_.inTensors.at(IN_HOLDER);               // IN_HOLDER
    ...
    atb::CreateOperation(lmHeadParm, &op);
...
atb::Status FusionModel::BindParamHostTensor(uint32_t nodeId)
{
    ...
    const uint32_t tokenOffsetTensorId = 14;
    const uint32_t seqLenTensorId = 15;
