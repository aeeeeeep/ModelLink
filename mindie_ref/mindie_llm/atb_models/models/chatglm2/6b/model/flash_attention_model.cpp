...
const int WEIGHT_COUNT_SPARSE_LAYER = 26;
const int WEIGHT_COUNT_FLOAT_LAYER = 7;
...
void ChatGlm2CommonModelFa::Param::FromString(const std::string &param)
{
    ...
    ATB_LOG(INFO) << "ChatGlm2QuantMixDecoderParallelModelFa param rmsNormEps:" << rmsNormEps
                  << ", numHeadsPerPartition:" << numHeadsPerPartition << ", hiddenSizePerHead:" << hiddenSizePerHead
                  << ", numGroupsPerPartition:" << numGroupsPerPartition << ", transKey:" << transKey
                  << ", layerNum:" << layerNum << ", residualAddScale:" << residualAddScale
                  << ", quantmodel" << quantmodel << ", correctNodeId" << correctNodeId << ", isEncoder" << isEncoder;
}

ChatGlm2CommonModelFa::ChatGlm2CommonModelFa(const std::string &param)
    : Model("ChatGlm2CommonModelFa", param)
{
    param_.FromString(param);
}

ChatGlm2CommonModelFa::~ChatGlm2CommonModelFa() {}

uint32_t ChatGlm2CommonModelFa::GetInputNum() const
{
    return graph_.inTensors.size();
}

uint32_t ChatGlm2CommonModelFa::GetOutputNum() const
{
    return graph_.outTensors.size();
}

atb::Status ChatGlm2CommonModelFa::InferShape(
    const std::vector<atb::TensorDesc> &inTensorDescs, std::vector<atb::TensorDesc> &outTensorDescs)
{
    ...
    for (size_t i = 1; i < outTensorDescs.size(); i++) {
        outTensorDescs.at(i) = keyTensorDesc;
...
atb::Status ChatGlm2CommonModelFa::ParseParam(const std::string &param)
{
    ...
    ATB_LOG(INFO) << "ChatGlm2CommonModelFa ParseParam tokenOffset:"
...
atb::Status ChatGlm2CommonModelFa::BindParamHostTensor(uint32_t nodeId)
{
    ...
    const uint32_t InttokenOffsetTensorId = 33;
    const uint32_t IntseqLenTensorId = 29;
