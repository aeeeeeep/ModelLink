...
template <class T> atb::Status MlpGateLayerBase(const MlpGateParam &param, atb::Operation **operation, T config)
{
    ...
    matmulUpParam.transposeB = param.transposeB;
    ...
    if (param.isPack) {
        ...
        matmulGateParam.transposeB = param.transposeB;
        matmulGateParam.hasBias = param.isBias;
    ...
    atb_speed::common::ParallelParam linearParallelParam = {param.rank, param.rankSize,  0, nullptr, false,
...
atb::Status MlpGateLayer(const MlpGateParam &param, atb::Operation **operation)
{
    if (param.isBias && param.isPack) {
        ...
        return MlpGateLayerBase(param, operation, MlpGateWithBias(6, 1, 4, 5)); // 6:in 1:out 4:inter 5:node
