 ...
 */
#include "fusion_pa_model.h"
#include "atb/atb_infer.h"
#include "atb_speed/log.h"
#include "models/llama2/70b/layer/fusion_pa_layer.h"
...
void FusionPAModel::Param::FromString(const std::string &param)
{
    ...
    ATB_LOG(INFO) << "Llama2_70B_FusionPAModel param rmsNormEps:" << rmsNormEps << ", headNum:" << headNum
...
FusionPAModel::FusionPAModel(const std::string &param) : Model("FusionPAModel", param)
{
    param_.FromString(param);
    modelName_ += param_.isPrefill ? "_Prefill" : "_Decoder";
}

FusionPAModel::~FusionPAModel() = default;

uint32_t FusionPAModel::GetInputNum() const { return graph_.inTensors.size(); }

uint32_t FusionPAModel::GetOutputNum() const { return graph_.outTensors.size(); }

atb::Status FusionPAModel::InferShape(const std::vector<atb::TensorDesc> &inTensorDescs,
                                      std::vector<atb::TensorDesc> &outTensorDescs)
{
    ATB_LOG(INFO) << "Enter LlamaFusionPAModel InferShape";
...
int64_t FusionPAModel::BuildGraph()
{
    ...
    ATB_LOG(INFO) << "FusionPAModel nodeSize is " << nodeSize;
    ...
    for (int layerId = 0; layerId < param_.layerNum; ++layerId) {
        ...
        atb_speed::llama2_70b::FusionPALayerParam layerParam;
        ...
        atb_speed::llama2_70b::FusionPALayer(layerParam, &op);
...
atb::Status FusionPAModel::ParseParam(const std::string &param)
{
    ...
    ATB_LOG(INFO) << "FusionPAModel ParseParam seqLen: " << seqLen_.capacity();
    return atb::NO_ERROR;
}

atb::Status FusionPAModel::BindParamHostTensor(uint32_t nodeId)
{
    ...
    const uint32_t seqLenTensorId = atb_speed::llama2_70b::FusionPALayerTensorId::IN_SEQLEN;
