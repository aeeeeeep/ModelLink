...
#include "models/mixtral/7b_dense/model/decoder_model.h"

namespace atb_speed {
namespace mixtralDense {

// Weight count
const int WEIGHT_COUNT_PER_LAYER = 46;
...
int64_t DecoderModel::BuildGraph()
{
    ...
    // idx: 12, Scaler, for one_hot
    int IN_ONE_HOT_ONE = inTensorIdx++;
    // idx: 13, Scaler, for one_hot
    int IN_ONE_HOT_ZERO = inTensorIdx++;
    // idx: 14, shape: FA: [batchSize, seqLen]
    int IN_FINAL_HIDDEN_STATE = inTensorIdx++;
    ...
    for (int layerId = 0; layerId < param_.numHiddenLayers; ++layerId) {
        ...
        atb_speed::mixtralDense::DecoderLayerParam layerParam;
        ...
        atb_speed::mixtralDense::DecoderLayer(layerParam, &op);
        ...
        layerNode.inTensors.at(inTensorId++) = &graph_.inTensors.at(IN_ONE_HOT_ONE);
        layerNode.inTensors.at(inTensorId++) = &graph_.inTensors.at(IN_ONE_HOT_ZERO);
        layerNode.inTensors.at(inTensorId++) = &graph_.inTensors.at(IN_FINAL_HIDDEN_STATE);
    ...
    ATB_LOG(INFO) << "DecoderModel build graph:fill intensors";
    ...
    ATB_LOG(INFO) << "DecoderModel build graph:finalNormNode end";
    ...
    ATB_LOG(INFO) << "DecoderModel build graph:create LMHead end";
