 ...
 */
#include "flash_attention_rope_antioutlier_layer.h"
...
enum FlashAttentionRopeAntiOutlierLayerTensorId : int {
    IN_HIDDENSTATES = 0,
    ...
    INTERMIDATE_SELFNORM_ADD_OUT,
    INTERMIDATE_MLPOUT,
};

static const uint64_t IN_TENSOR_COUNT = 27;             // layer层输入tensor数量
static const uint64_t OUT_TENSOR_COUNT = 1;             // layer层输出tensor数量
static const uint64_t INTERMEDIATE_TENSOR_COUNT = 13;   // layer层中间tensor数量
static const uint64_t NODE_COUNT = 13;                  // layer层图节点数量
    ...
    atb::Operation **operation)
{
    ...
    atb::Node &selfNormAddNode = opGraph.nodes.at(nodeId++);
    atb::Node &mlpNode = opGraph.nodes.at(nodeId++);
    atb::Node &mlpResidualAddNode = opGraph.nodes.at(nodeId++);

    ATB_LOG(INFO) << __func__ << " nodeId: " << nodeId;
...
FlashAttentionRopeAntiOutlierLayerBinder::FlashAttentionRopeAntiOutlierLayerBinder() = default;

FlashAttentionRopeAntiOutlierLayerBinder::~FlashAttentionRopeAntiOutlierLayerBinder() = default;

void FlashAttentionRopeAntiOutlierLayerBinder::ParseParam(const nlohmann::json &paramJson)
{
    tokenOffset_.clear();
...
void FlashAttentionRopeAntiOutlierLayerBinder::BindTensor(atb::VariantPack &variantPack)
{
...
atb::Operation *CreateFlashAttentionRopeAntiOutlierLayer(const nlohmann::json &paramJson)
{
    ...
    atb::Operation *op;
    atb_speed::internlm_20b::FlashAttentionRopeAntiOutlierLayer(
