...
enum LayerEncoderAdapterTensorId : int {
    IN_HIDDENSTATES = 0,
    IN_NORMWEIGHT,
    IN_QWEIGHT,
    IN_QBIAS,
...
atb::Status EncoderAdapterLayer(const LayerParam &param, atb::Operation **operation)
{
    ...
    atb_speed::llama_adapter::SelfAttentionCrossEnAdapter(selfAttentionCrossParam, &selfAttentionNode.operation);
    ...
    selfAttentionNode.inTensorReshapeFuncs.at(3) = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
        newShape.dimNum = 4;
        newShape.dims[0] = oldShape.dims[0];
        newShape.dims[1] = oldShape.dims[1];
        newShape.dims[2] = param.headNum;
    ...
    selfAttentionNode.inTensorReshapeFuncs.at(4) = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
        newShape.dimNum = 4;
        newShape.dims[0] = oldShape.dims[0];
        newShape.dims[1] = oldShape.dims[1];
        newShape.dims[2] = param.headNum;
