...
#include "models/codellama/34b/operation/rope.h"
...
atb::Status FlashAttentionRopeLayer(const FlashAttentionRopeLayerParam &param, atb::Operation **operation)
{
    ...
    atb_speed::codellama_34b::RopeParam ropeParam;
    ropeParam.rotaryCoeff = ROTARY_COEFF; // 旋转系数
    ropeParam.headNum = param.headNum;
    ropeParam.kvHeadNum = param.kvHeadNum;
    atb_speed::codellama_34b::Rope(ropeParam, &ropeNode.operation);
    ...
    selfAttentionKvCacheNode.inTensorReshapeFuncs.at(SELF_ATTENTION_V_INPUT_INDEX) = [=](const atb::Dims &oldShape,
        ...
        newShape.dims[newShapeDimIndex++] = oldShape.dims[oldShapeDimIndex++];
        newShape.dims[newShapeDimIndex++] = param.kvHeadNum;
        newShape.dims[newShapeDimIndex++] = oldShape.dims[oldShapeDimIndex++] / param.kvHeadNum;
    };
...
void from_json(const nlohmann::json &paramJson, const FlashAttentionRopeLayerParam &param)
{
    ...
    if (paramJson.contains("kvHeadNum")) {
        paramJson.at("kvHeadNum").get_to(param.kvHeadNum);
    }
}
...
atb::Operation *CreateFlashAttentionRopeLayer(const nlohmann::json &paramJson)
{
    ...
    atb::Operation *op;
    atb_speed::codellama_34b::FlashAttentionRopeLayer(paramJson.get<FlashAttentionRopeLayerParam>(), &op);
    return op;
}
} // namespace codellama_34b
} // namespace atb_speed