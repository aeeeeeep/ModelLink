    ...
    const ParallelType parallelType)
{
    ...
    if (param_.isBias) {
        ...
        addNode.inTensorIds = {
            param_.rankSize > 1 ? config.INTERMIDATE_ALLREDUCEOUT : config.INTERMIDATE_MATMULOUT,
            config.IN_BIAS
        };
    ...
    if (parallelType == ROW_PARALLEL) {
        opGraph.inferShapeFunc = [=](const atb::SVector<atb::TensorDesc> &inTensorDescs,
            ...
            if (param_.transposeB) {
                outTensorDescs.at(0).shape.dims[dimNum-1] = inTensorDescs.at(1).shape.dims[1]; // dim 2
            } else {
                outTensorDescs.at(0).shape.dims[dimNum-1] = inTensorDescs.at(1).shape.dims[0]; // dim 2
            }
        ...
        opGraph.inferShapeFunc = [=](const atb::SVector<atb::TensorDesc> &inTensorDescs,
            ...
            outTensorDescs.at(0).format = inTensorDescs.at(0).format;
            auto dimNum = inTensorDescs.at(0).shape.dimNum;
            outTensorDescs.at(0).shape.dimNum = dimNum + 1; 
            ...
            if (param_.transposeB) {
                outTensorDescs.at(0).shape.dims[dimNum] = inTensorDescs.at(1).shape.dims[1]; // dim 3
            } else {
                outTensorDescs.at(0).shape.dims[dimNum] = inTensorDescs.at(1).shape.dims[0]; // dim 3
            }
...
atb::Status ParallelLinear(const ParallelParam &param_, atb::Operation **operation, const ParallelType parallelType)
{
    if (param_.isBias && (param_.rankSize > 1)) {
        return ParallelLinearBase(param_, operation,
            LinearWithBiasAndParallel(3, 1, 2, 3), parallelType); // 3:in 1:out 2:inter 3:node
    } else if (param_.isBias) {
        return ParallelLinearBase(param_, operation,
            LinearWithBias(3, 1, 1, 2), parallelType); // 3:in 1:out 1:inter 2:node
    } else if (param_.rankSize > 1) {
        return ParallelLinearBase(param_, operation,
            LinearWithParallel(2, 1, 1, 2), parallelType); // 2:in 1:out 1:inter 2:node
    } else {
        return ParallelLinearBase(param_, operation,
            LinearOnly(2, 1, 0, 1), parallelType); // 2:in 1:out 0:inter 1:node
    }
}
