...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
                 max_seq_len=512, max_batch_size=15,
                 clip_model='ViT-L/14',
    ...
    def init_acl_weight(self):
        ...
        for i in range(self.num_layers):
            ...
            weights_t.append(weights_layer[str_keys + "attention.wq.bias"])
            weights_t.append(weights_layer[str_keys + "attention.wk.weight"])
            weights_t.append(weights_layer[str_keys + "attention.wv.weight"])
            weights_t.append(weights_layer[str_keys + "attention.gate"])
            weights_t.append(weights_layer[str_keys + "attention.wo.weight"])
            weights_t.append(weights_layer[str_keys + "attention.wo.bias"])
            weights_t.append(weights_layer[str_keys + "ffn_norm.weight"])
            weights_t.append(weights_layer[str_keys + "feed_forward.w1.weight"])
            weights_t.append(weights_layer[str_keys + "feed_forward.w1.bias"])
            weights_t.append(weights_layer[str_keys + "feed_forward.w2.weight"])
            weights_t.append(weights_layer[str_keys + "feed_forward.w2.bias"])
            weights_t.append(weights_layer[str_keys + "feed_forward.w3.weight"])
            weights_t.append(weights_layer[str_keys + "feed_forward.w3.bias"])
    ...
    def init_acl_encoder_param(self, hidden_states, adapter_in, freqs_cis, mask=None):
        hidden_states = hidden_states.half().npu()
    ...
    def forward_inference(self, tokens, start_pos: int):
        ...
        freqs_cis_real = torch.view_as_real(freqs_cis).npu()
...
def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts', max_seq_len=512,
    ...
    model = LLaMA_adapter(
        ...
        max_seq_len=max_seq_len, max_batch_size=15,
        clip_model='ViT-L/14',
        v_embed_dim=768, v_depth=8,
        v_num_heads=16, v_mlp_ratio=4.0,
        query_len=10, query_layer=31,
