...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            attention_mask_atb = self._prepare_decoder_attention_mask(
        ...
        if past_key_values is None: # 首token
            ...
            self.attention_mask_max = torch.full(
                (self.batch_num, self.max_sequence_length, self.max_sequence_length), 0, device='npu', dtype=torch.half)
            if attention_mask is not None:
                attention_mask_acl = attention_mask[:,0,:,:].to(torch.half)
                self.attention_mask_max[:self.batch_num, :self.token_num, :attention_mask_acl.size()[-1]] += attention_mask_acl
                self.attention_mask_max = self.attention_mask_max.view(self.batch_num, self.max_sequence_length, self.max_sequence_length // 16, 16).transpose(1, 2)
            ...
            if attention_mask is not None:
                ...
                self.attention_mask_max[:self.batch_num, :self.nz_dim, :attention_mask_acl.size()[-1]] += attention_mask_acl.unsqueeze(-2).repeat(1, self.nz_dim, 1)
                self.attention_mask_max = self.attention_mask_max.view(self.batch_num, self.max_sequence_length, self.max_sequence_length // self.nz_dim, self.nz_dim).transpose(1, 2)
