...
class TestNPUDropoutAddLayerNorm(unittest.TestCase):
    ...
    def dropout_add_layer_norm_prenorm_training(self, batch_size, seqlen, epsilon):
        ...
        for item in input_training:
            ...
            self.assertTrue((x0.grad.cpu() - x0_ref.grad).abs().max() <= 4 * (x0_pt.grad - x0_ref.grad).abs().max() + 1e-4)
    ...
    def dropout_add_rms_norm_eval(self, batch_size, seqlen, epsilon):
        ...
        for item in input_eval:
            ...
            self.assertTrue((out.cpu() - out_ref).abs().max() <= 4 * (out_pt.to(item[1]) - out_ref).abs().max() + 1e-4)
