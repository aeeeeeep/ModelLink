...
from modellink.tasks.inference.text_generation.module import MegatronModuleForCausalLM
...
class TestEvaluation(DistributedTest):
    ...
    def test_ceval_evaluation(self):
        ...
        from modellink.tasks.evaluation.eval_impl.template import CEVAL_TEMPLATE_DIR
        ...
        template_dir = os.path.join(base_dir, CEVAL_TEMPLATE_DIR)
        ...
        for file in tqdm.tqdm(os.listdir(test_dir)):
            ...
            for index, row in data_df_test.iterrows():
                ...
                instruction = instruction_template.format(few_shot_examples=ceval_few_shot_template[subject_name],
        ...
        if torch.distributed.get_rank() == 0:
            try:
                ...
                raise e
            print(final_acc)
            assert_judge(abs(final_acc - 0.661) < 0.03)