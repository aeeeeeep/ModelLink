        ...
        for file in tqdm.tqdm(os.listdir(self.test_dir)):
            ...
            if rank == 0:
                logger.info("%s acc = %d/%d=%e", subject_name, acc_n, len(agi_question_list),
                             acc_n / len(agi_question_list))
                total_n += len(agi_question_list)
                total_acc_n += acc_n
                answer_result[subject_name] = subject_result
                score_datas.append([subject_name, len(agi_question_list), acc_n / len(agi_question_list)])
        if rank == 0:
            logger.info("AGIEval acc = %d/%d=%e", total_acc_n, total_n, total_acc_n / total_n)
