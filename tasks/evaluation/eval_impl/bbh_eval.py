...
class BBHEval(DatasetEval):
    ...
    def eval(self, llm_chat: LlmChat) -> (dict, pd.DataFrame):
        ...
        for file in tqdm.tqdm(os.listdir(self.test_dir)):
            ...
            if rank == 0:
                logging.info("%s acc = %d/%d=%e", subject_name, acc_n, len(bbh_dataset['examples']),
                             acc_n / len(bbh_dataset['examples']))
                total_n += len(bbh_dataset['examples'])
                total_acc_n += acc_n
                answer_result[subject_name] = subject_result
                score_datas.append([subject_name, len(bbh_dataset['examples']), acc_n / len(bbh_dataset['examples'])])
        if rank == 0:
            logger.info("bbh acc = %d/%d=%e", total_acc_n, total_n, total_acc_n / total_n)
