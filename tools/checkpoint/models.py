...
class ModelBase(abc.ABC):
    ...
    def __register_functions(self):
        ...
        def func_generator_get_bais(value):
            def func(self, **kwargs):
                return get_obj(self, value, **kwargs).weight.data
            return func
        ...
        def func_generator_set_weight(value):
            def func(self, **kwargs):
                return get_obj(self, value, **kwargs).weight.data.copy_(kwargs["data"])
            return func
        ...
        if self.module_mapping:
            for key, value in self.module_mapping.items():
                ...
                setattr(self, "get_" + key + "_bias", func_generator_get_bais(value).__get__(self, ModelBase))
...
class HuggingfaceModel(ModelBase):
    ...
    def read_model_cfg(self):
        ...
        for key, value in config.items():
            if "__base__" in value:
                base_key = value.pop("__base__")
                base_config = copy.deepcopy(resolved[base_key])
                resolved[key] = self.merge_dict(base_config, value)
            else:
                resolved[key] = value
        return resolved['model_mappings']
    ...
    def get_args(self):
        ...
        config_key_mapping = self.model_cfg['base']['config_hf_key_mapping']
        config_value = self.model_cfg['base']['config_set_value']
    ...
    def _get_module_mapping(self):
        self.module_mapping = self.model_cfg['base']['model_hf_key_mapping']
    ...
    def _get_layers_self_attention_linear_qkv_module(self, layer_idx=0):
        ...
        if qkv_type == "type1":
            ...
            if 0 and self.args['add_qkv_bias']:
    ...
    def merge_dict(self, base, override):
        """递归地合并两个字典，override中的值会覆盖base中的值"""
