...
def save_huggingface_llama(args, model, model_args):
    ...
    for name_param_m in model.named_parameters():
        ...
        repeats = nh // ng
        if name_param_m[0] == "embedding.word_embeddings.weight":
            hf2mg_map["model.embed_tokens.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.pre_mlp_layernorm.weight":
            hf2mg_map[f"model.layers.{layer_num}.post_attention_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.input_layernorm.weight":
            hf2mg_map[f"model.layers.{layer_num}.input_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.pre_mlp_layernorm.weight":
            hf2mg_map[f"model.layers.{layer_num}.post_attention_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_qkv.weight":
            ...
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_qkv.bias":
            ...
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_proj.bias":
            hf2mg_map[f"model.layers.{layer_num}.self_attn.linear_proj.bias"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_proj.weight":
            hf2mg_map[f"model.layers.{layer_num}.self_attn.o_proj.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.mlp.linear_fc1.weight":
            ...
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.mlp.linear_fc2.weight":
            hf2mg_map[f"model.layers.{layer_num}.mlp.down_proj.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == "decoder.final_layernorm.weight":
            hf2mg_map[f"model.norm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == "output_layer.weight":
...
def save_huggingface_chatglm3(args, model, model_args):
    ...
    for name_param_m in model.named_parameters():
        ...
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_qkv.weight":
            ...
            vw = qkv_weight[:, repeats + 1 :, ...].reshape(-1, w)
            qkv = torch.cat((qw, kw, vw), dim=0)
            hf2mg_map[f"transformer.decoder.layers.{layer_num}.self_attention.linear_qkv.weight"] = qkv
            continue
        # linear_qkv bias
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_qkv.bias":
            bias_weight = name_param_m[1].reshape(
            ...
            kw = bias_weight[:, repeats : repeats + 1, ...].reshape(-1)
            vw = bias_weight[:, repeats + 1 :, ...].reshape(-1)
            hf2mg_map[f"transformer.decoder.layers.{layer_num}.self_attention.linear_qkv.bias"] = torch.cat(
                [qw, kw, vw], dim=0
            )
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.self_attention.linear_proj.weight":
            hf2mg_map[f"transformer.decoder.layers.{layer_num}.self_attention.linear_proj.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.pre_mlp_layernorm.weight":
            hf2mg_map[f"transformer.decoder.layers.{layer_num}.post_attention_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.mlp.linear_fc1.weight":
            hf2mg_map[f"transformer.decoder.layers.{layer_num}.mlp.linear_fc1.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"decoder.layers.{layer_num}.mlp.linear_fc2.weight":
            hf2mg_map[f"transformer.decoder.layers.{layer_num}.mlp.linear_fc2.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == "decoder.final_layernorm.weight":
            hf2mg_map[f"transformer.decoder.final_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == "output_layer.weight":
...
def save_model_checkpoint(queue, args):
    ...
    for tp_rank, model in enumerate(models):
        ...
        if orig_word_embed_n_w is not None:
            model.embedding.word_embeddings.norm.weight.data.copy_(orig_word_embed_n_w)
            model.embedding.word_embeddings.norm.bias.data.copy_(orig_word_embed_n_b)
        if pos_embed is not None:
            model.embedding.position_embeddings.weight.data.copy_(pos_embed)
        else:
            if hasattr(model.embedding, 'position_embeddings'):
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for vpp_rank in range(vp_size):
            for layer in range(len(models[0].decoder.layers) // vp_size):
                if args.num_layers_per_virtual_pipeline_stage and args.save_model_type == 'megatron':
                ...
                for tp_rank in range(args.target_tensor_parallel_size):
                    if args.num_layers_per_virtual_pipeline_stage and args.save_model_type == 'megatron':
                        l = models[tp_rank].decoder.layers[
        ...
        if post_process:
            ...
            if msg != "done" and msg["name"] == "pooler":
                ...
                for tp_rank in range(args.target_tensor_parallel_size):
                    models[tp_rank].pooler.linear_proj.weight.data.copy_(pooler_weight)
                    models[tp_rank].pooler.linear_proj.bias.data.copy_(pooler_bias)
                del pooler_weight
                del pooler_bias
                check_message(msg)
                msg = queue_get()
            ...
            if msg != "done" and msg["name"] == "lm head":
                ...
                for tp_rank in range(args.target_tensor_parallel_size):
                    models[tp_rank].lm_head.linear_proj.weight.data.copy_(lm_head_dense_weight)
                    models[tp_rank].lm_head.linear_proj.bias.data.copy_(lm_head_dense_bias)
