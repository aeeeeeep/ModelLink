...
import sys
import types
# 将Megatron-LM路径添加到python path中
megatron_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(megatron_dir)
import torch
...
def add_transformers_checkpoint_args(parser):
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "The num of iterations wanted to start with in training"
        ),
    )
    parser.add_argument(
        "--true-vocab-size",
    ...
    parser.add_argument(
        ...
        choices=["gpt", "llama"],
        help=(
            "The model type of the pre-trained model to save. Current supported model: GPT, llama"
        ),
    )
    parser.add_argument(
...
gpt_transformers_to_megatron = {v[1:-1]: k for k, v in gpt_megatron_to_transformers.items()}

llama_megatron_to_transformers = {
    "attention.dense": ".self_attn.o_proj.",
    "self_attention.dense": ".self_attn.o_proj.",
    "mlp.dense_4h_to_h": ".mlp.down_proj.",
}
llama_transformers_to_megatron = {v[1:-1]: k for k, v in llama_megatron_to_transformers.items()}
...
tensor_parallel_params = [
    ...
    "mlp.dense_h_to_4h_gate.weight",
    "mlp.dense_h_to_4h_gate.bias",
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    print(f"true vocab size: {vocab_size}")
    ...
    if args.model_type == "gpt":
        ...
        config = LlamaConfig(
            ...
            hidden_act="silu",
            use_cache=True,
            rotary_percent=megatron_args.rotary_percent,
    ...
    if args.untie_embeddings_and_output_weights:
        ...
        lm_heads = lm_heads[:vocab_size].to(dtype)
    ...
    if args.model_type == "gpt":
        ...
        transformer_variable_name = "transformer"
        layer_variable_name = "h"
        attention_variable_name = "attn"
        input_norm_variable_name = "ln_1"
        post_attention_norm_variable_name = "ln_2"
        final_norm_variable_name = "ln_f"
        ...
        num_layers = config.num_hidden_layers // pp_size
        transformer_variable_name = "model"
        layer_variable_name = "layers"
        attention_variable_name = "self_attn"
        input_norm_variable_name = "input_layernorm"
        post_attention_norm_variable_name = "post_attention_layernorm"
        final_norm_variable_name = "norm"
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            layer_name = f"{transformer_variable_name}.{layer_variable_name}.{layer_idx}"
            ...
            if op_name.endswith("layernorm"):
                if "inter" in op_name:
                    continue
                ln_name = input_norm_variable_name if op_name.startswith("input") else post_attention_norm_variable_name
            ...
            # Transpose the QKV matrix.
                ...
                # )
                # output_state_dict[layer_name + f".{attention_variable_name}.bias"] = causal_mask

                # Insert a "dummy" tensor for masked_bias.
                # masked_bias = torch.tensor(-1e4, dtype=dtype)
                # output_state_dict[layer_name + f".{attention_variable_name}.masked_bias"] = masked_bias
                ...
                if args.model_type == "gpt":
                    ...
                    output_state_dict[layer_name + f".{attention_variable_name}.q_proj.weight"] = q
                    output_state_dict[layer_name + f".{attention_variable_name}.k_proj.weight"] = k
                    output_state_dict[layer_name + f".{attention_variable_name}.v_proj.weight"] = v
            ...
            # Transpose the bias.
                ...
                if args.model_type == "gpt":
                    ...
                    q, k, v = torch.chunk(out_val, 3, dim=-1)
                    output_state_dict[layer_name + f".{attention_variable_name}.q_proj.bias"] = q
                    output_state_dict[layer_name + f".{attention_variable_name}.k_proj.bias"] = k
                    output_state_dict[layer_name + f".{attention_variable_name}.v_proj.bias"] = v

            elif op_name == "mlp.dense_h_to_4h":
                if args.model_type == "gpt":
                    out_name = gpt_megatron_to_transformers[op_name]
                    if weight_or_bias == "weight":
                        output_state_dict[layer_name + out_name + "weight"] = params.transpose(0, 1)
                    else:
                        output_state_dict[layer_name + out_name + "bias"] = params.transpose(0, 1)
                elif args.model_type == "llama":
                    if weight_or_bias == "weight":
                        output_state_dict[layer_name + ".mlp.up_proj." + "weight"] = params
                    else:
                        output_state_dict[layer_name + ".mlp.up_proj." + "bias"] = params
            elif op_name == "mlp.dense_h_to_4h_gate":
                if weight_or_bias == "weight":
                    output_state_dict[layer_name + ".mlp.gate_proj." + "weight"] = params
                else:
                    output_state_dict[layer_name + ".mlp.gate_proj." + "bias"] = params
            # Transpose the weights.
                ...
                if args.model_type == "gpt":
                   ...
                   out_name = llama_megatron_to_transformers[op_name]
    ...
    if num_layers != (layer_idx + 1):
        raise ValueError(f"Expected {num_layers} layers but found {layer_idx + 1}")

    # The final layernorm.
    print("Converting final layernorm")
    params = get_element_from_dict_by_path(tp_state_dicts[0], str(path))
    output_state_dict[f"{transformer_variable_name}.{final_norm_variable_name}.weight"] = params["final_layernorm.weight"].to(dtype)
    if args.model_type == "gpt":
        output_state_dict[f"{transformer_variable_name}.{final_norm_variable_name}.bias"] = params["final_layernorm.bias"].to(dtype)

    # For LM head, transformers' wants the matrix to weight embeddings.
    print("Converting LM head")
    if args.untie_embeddings_and_output_weights:
        output_state_dict["lm_head.weight"] = lm_heads.to(dtype)
    else:
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    if args.untie_embeddings_and_output_weights:
        llm_head = state_dict["lm_head.weight"].to(dtype)
        # Cut out extra padding we don't need
        if orig_vocab_size > padded_vocab_size:
            full_llm_head = llm_head[0:padded_vocab_size, :]
        # Expanding embedding to larger size by replicating final entry
        elif orig_vocab_size < padded_vocab_size:
            padding_size = padded_vocab_size - orig_vocab_size
            full_llm_head = torch.cat((llm_head, llm_head[-1].unsqueeze(0).expand(padding_size, -1)))
        # Same size!
        else:
            full_llm_head = llm_head
        out_llm_head = torch.chunk(full_llm_head, args.target_tensor_model_parallel_size, dim=0)
    ...
    layer_re = re.compile(r"%s.%s\.(\d+)\.([a-z0-9_.]+)\.([a-z]+)"%(transformer_variable_name, layer_variable_name))
    # The number of heads.
    if args.model_type == "llama":
        heads = config.num_attention_heads
        # The hidden_size per head.
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        for layer in range(num_layers):
            ...
            layers_to_copy = [
                ...
                if layer_name.startswith(f"{transformer_variable_name}.{layer_variable_name}.{pp_layer_id}.")
            ]
            qkv_matrix={}
            swish_proj={}
            for layer_name in layers_to_copy:
                ...
                # handle layernorm
                if args.model_type == "llama":
                    if op_name.startswith(final_norm_variable_name) or "layernorm" in op_name:
                        layer_name = f"layers.{layer}.{op_name}.{weight_or_bias}"
                    # handle attention K, V, Q weights
                    elif op_name.startswith(attention_variable_name) and "o_proj" not in op_name and weight_or_bias == "weight":
                        # transformers stores D X (3*D) but Megatron-LM expects (3*D) X D.
                        if len(qkv_matrix) < 2:
                            qkv_matrix[op_name.split(".")[-1]] = params
                            continue
                        else:
                            qkv_matrix[op_name.split(".")[-1]] = params
                            params = torch.cat([
                    ...
                    # handle swish mlp weights
                    elif op_name.endswith("gate_proj"):
                        layer_name = f"layers.{layer}.mlp.dense_h_to_4h_gate.{weight_or_bias}"
                    elif op_name.endswith("up_proj"):
                        layer_name = f"layers.{layer}.mlp.dense_h_to_4h.{weight_or_bias}"
                    # handle attention and mlp weights
                    elif weight_or_bias == "weight":
                        out_name = llama_transformers_to_megatron.get(op_name, None)
                        if out_name is None:
                            continue
                    ...
                    # handle attention and mlp weights
                        ...
                        out_name = gpt_transformers_to_megatron.get(op_name, None)
                        if out_name is None:
                            continue
                ...
                if op_name + "." + weight_or_bias in tensor_parallel_params:
                    shard_flag = True
                    dim = 1 if op_name in ["attn.c_proj", "mlp.c_proj", "self_attn.o_proj", "mlp.down_proj"] else 0
                    params = torch.chunk(params, args.target_tensor_model_parallel_size, dim=dim)
                
                # 对llama的qkv合并矩阵进行tp切分
                if op_name.startswith(attention_variable_name) and "o_proj" not in op_name and len(qkv_matrix) == 3:
                    shard_flag = True
                    params = torch.chunk(params, args.target_tensor_model_parallel_size, dim=0)
                    
                # 对llama的mlp swish矩阵参数进行tp切分
                if op_name.endswith("gate_proj") or op_name.endswith("up_proj"):
                    shard_flag = True
                    params = torch.chunk(params, args.target_tensor_model_parallel_size, dim=0)
                ...
                for i in range(args.target_tensor_model_parallel_size):
                    ...
                    params_dict[layer_name] = (
                        params[i] if shard_flag else params
                    )
        ...
        if pp_rank == args.target_pipeline_model_parallel_size - 1:
            ...
            for weight_or_bias in ["weight", "bias"]:
                if args.model_type == "llama":
                    params = state_dict[f"model.{final_norm_variable_name}.weight"].to(dtype)
                    layer_name = f"final_layernorm.weight"
                else:
            ...
            for i in range(args.target_tensor_model_parallel_size):
                if not args.untie_embeddings_and_output_weights:
                    ...
                    params_dict["weight"] = out_llm_head[i]

        # saving the state dict as per the tp_rank and pp_rank
        for tp_rank in range(args.target_tensor_model_parallel_size):
            output_state_dict[tp_rank]["checkpoint_version"] = 3.0
            output_state_dict[tp_rank]["args"] = margs
            output_state_dict[tp_rank]["iteration"] = args.iteration
            checkpoint_dir = (
