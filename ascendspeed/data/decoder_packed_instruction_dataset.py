...
from ascendspeed import print_rank_0, mpu
...
from ascendspeed.data.instruction_dataset import InstructionDataset, get_packed_indexed_dataset
...
class DecoderPackedMTFDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.mtf_dataset = InstructionDataset(name=name, data_prefix=data_prefix, data_impl=data_impl, skip_warmup=skip_warmup, documents=documents)
    ...
    def __getitem__(self, idx):
        ...
        return {
            "input_ids": self._pad_token(item["input_ids"], self.pad_token, np.int64),
            "attention_mask": self._pad_token(item["attention_mask"], 0, np.int64),
            "labels": self._pad_token(item["labels"], -100, np.int64),
        }
