...
class Bmm2Function(torch.autograd.Function):
    ...
    def forward(ctx, a, b, seqlen, head_num):
        ...
        c = ascendspeed_te_ops.npu_strided_batch_matmul(a, b, transA, transB, m, k, n, lda, ldb, ldc, strideA, strideB, strideC, batch, head_num).view(sum(seqlen), hidden_size)
    ...
    def backward(ctx, grad_out):
        ...
        grad_x = ascendspeed_te_ops.npu_strided_batch_matmul(grad_out, y, transA, transB, m, k, n, lda, ldb, ldc, strideA, strideB, strideC, batch, head_num)
        ...
        strideB = [head_size] * batch
        strideC = [head_size] * batch
        grad_y = ascendspeed_te_ops.npu_strided_batch_matmul(x, grad_out, transA, transB, m, k, n, lda, ldb, ldc, strideA, strideB, strideC, batch, head_num).view(sum(seqlen), hidden_size)
