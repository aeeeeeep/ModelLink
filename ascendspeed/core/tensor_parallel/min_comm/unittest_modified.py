...
def print_tensor_value(name, value, device_id, n):
...
def initialize(rank, world_size, initialize_cc_from_cfg):
    global tp_group
...
def global_run(rank, world_size, customized_m, comm_name, compute_first):
    ...
    from ascendspeed.mpu.min_comm.cc_utils import shuffle_as_cc_all_gather, shuffle_as_cc_reduce_scatter
    get_CCL_type = {
    ...
    get_CCL_fcn = {
        CommunicationType.ALL_GATHER: ut_gather_along_first_dim,
        CommunicationType.REDUCE_SCATTER: ut_reduce_scatter_along_first_dim,
        CommunicationType.ALL_REDUCE: ut_reduce
    }
    comm_type = get_CCL_type[comm_name]
    initialize(rank, world_size, initialize_cc_from_cfg)
    input_parallel = generate_tensors(customized_m)
    global min_comm_config
    min_comm_config.tp_group_fcn = lambda: tp_group
    min_comm_config.tp_world_size_fcn = lambda: world_size
    min_comm_config.register_mappings(ut_reduce, ut_reduce_scatter_along_first_dim, ut_gather_along_first_dim)
    cc_output_parallel = get_cc_output_parallel(input_parallel, comm_type, compute_first, CCParallel, CommunicationType)
    original_output_parallel = get_original_output_parallel(input_parallel, get_CCL_fcn[comm_type], compute_first)
