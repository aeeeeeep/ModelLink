...
在框架中注册 cc：在 ascendspeed/initialize.py 中按照如下示例 import 并调用 initialize_cc_from_cfg 函数；
...
这部分逻辑同样支持独立地开启与关闭，其开关通过 ./user_config.py 中的 cc_cfgs 字典中对应的字段进行设置。


### 根据Matmul Shape自定义CC切分份数

有时候会出现一部分 Matmul 进行计算通信并行有收益，而一部分没有收益的情况。这时候我们可以手动根据 shape 来关闭那部分 Matmul
的计算通信并行。当然，如果希望让一部分 Matmul 使用非 CC_PARALLEL_NUM 设置的值作为其切分份数，也可以通过这个方法配置。该配置同样通过 ./user_config.py 中的 cc_cfgs 字典中对应的字段进行设置。
...
该修改同样通过 ./user_config.py 中的 cc_cfgs 字典中对应的字段进行设置（默认开启重计算）。
...
![image-20231011114506930](images/image_cc_good_compute_first.png)Example 2：下图为先通信后计算的并行（左边为不开启计算通信并行，右边为开启并行数为
2 的计算通信并行)。
...
假设被切分的那个Matmul在不开启计算通信并行，即未切分时的 shape 为 [4096,1024] 乘以 [1024, 8192]，则需要通过 ./user_config.py 中的 cc_cfgs 字典中对应的字段进行设置。
