...
2. 任意并行切分策略的Megatron权重 --> 任意并行切分策略的Megatron权重
    ***（该场景一般用于重新配置切分后模型的权重，比如在双机16卡 EP2-PP8策略下训练完了，想在单机8卡 PP8上进行推理）***
    ```bash
    cd ModelLink
    # 修改 ascend-toolkit 路径
    source /usr/local/Ascend/ascend-toolkit/set_env.sh
...
| :--: | :-------: | :----: |:-------------------:|:---------------------:|:-------------------: |
| NPUs | Mixtral-8x7B |  1024  |  3.136  |  802.73  |  40.82  |
| 参考 | Mixtral-8x7B |  1024  |     |     |     |

#### 精度

NPU VS 参考 loss

![NPU-LOSS](../../sources/images/mixtral/mixtral-8x7b-ep8pp2mbs1gbs128-loss-absolute.png)

相对误差

![NPU-Relative-Error](../../sources/images/mixtral/mixtral-8x7b-tp8pp1mbs4gbs16-loss-relative.png)
...
# 注意
该文档中使用的Mixtral-8x7B-v0.1模型为L0模型，只有续写能力，推理不带任何模板并且容易出现回答不停止的情况。
...
CHECKPOINT=../Mixtral-8x7B-v0.1-tp1pp8        #模型路径
# 配置任务和数据集路径
DATA_PATH="./gms8k/test/"
TASK="gms8k"
```
